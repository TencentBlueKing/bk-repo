...
| --- | --- | --- | --- | --- |
| Default 写失败 | 双写期 | 否 | 主路径失败，直接返回失败，不写 Offload | 单次写失败 |
| Default 写成功、Offload 写失败 | 双写期 | 是（异步补偿） | 记录补偿任务，主写保持成功 | Offload 最终一致延迟 |
| Default 写成功、Offload 超时 | 双写期 | 是（异步补偿） | 记录补偿任务，主写保持成功 | Offload 最终一致延迟 |
