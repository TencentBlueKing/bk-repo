...
| 散发查询（`pageBySha256`，无 `projectId`） | — | — | 各实例（`uri` 专用连接池并发查询）后合并去重 | 最终一致，各实例 Secondary 延迟可能不同 |
...
| 3（历史迁移中） | ✅ 自动（停止 mongorestore） | 无需 | 低 |
| 4a（ROUTED，Default 未清理，无新增写） | ✅ 自动 | 无需 | 低 |
| 4b（ROUTED，Default 未清理，有新写入） | ⚠️ 配置自动 + DBA 手动 | Offload→Default 反向同步增量 | 中 |
| 5a（CLEANUP_READY，清理进行中） | ⚠️ 配置自动 + DBA 手动 | 停止清理 + 按进度反向同步 | 中 |
| **5b（CLEANED，清理已完成）** | **❌ fail-fast 409** | DBA 全量反向 mongodump/restore | **高** |

> **关键结论**：只有阶段 5b（`CLEANED`）API 回滚不可用。一旦 Default 上 artifact_oplog 数据已清理完毕、磁盘已回收，API 回滚接口返回 409，后续须走 DBA SOP 全量反向同步。因此 **ROUTED → CLEANUP_READY 的推进须经充分的稳定运行观察期（建议 ≥ 1 周）**，确认无需回滚后再执行清理。
...
| 4b（ROUTED 后 Offload 有新写入） | 配置回滚 + 文档提示 | DBA 按 SOP 反向同步新增数据 |
...
> **核心结论**：阶段 4 的「Default 未清理」≠「Default 数据完整」。切流后 Default 已停写，回退到 Default 必然缺失切流期间增量。只有在「Offload 短期可恢复但需要让业务先跑起来」的场景下，才值得接受这个缺口切回 Default。
...
| ROUTED 后发现迁移决策错误 | ❌ 不应回滚 | 应计划性反向迁移（全量 dump/restore Offload→Default），非紧急回滚 |
...
| 补偿调度器自身宕机 | 是（K8s 自动重启 Pod 后从任务表恢复） | 重启后从任务表恢复，继续消费 | 补偿延迟 |
...
| 新旧 Pod 并存时配置不一致 | 否 | `kubectl rollout status` 确认 100% Pod 一致后恢复；持续不一致则立即回滚 `routing-state=OFF` | 部分请求路由错误 |
...
- `POST /migration/binding` 支持 `businessId` 参数 — 为组内所有项目批量创建 `mongo_migration_sync_state` 记录。后续 `start` / `dual-write` / `route` / `cleanup` 均以 `projectId` 为操作粒度。
- 迁移中禁止向组内新增项目；ROUTED 后新项目须显式绑定（走 §3.5.1b 增量流程）。
- `routing-state=ROUTED` 时，仅 `phase >= ROUTED` 的项目路由到 Heavy；未迁移项目虽命中 `project-routing`（组展开），但 `phase < ROUTED` 时路由回落 Default（§3.6.2 路由决策矩阵）。
...
flowchart TD
    ...
    BIND["POST /migration/binding\nruleName=node, businessId=biz-ci\n→ 批量创建 A/B/C 的 migration_sync_state\n→ business-routing 写入 Consul（project-routing 展开）"]

    BIND --> SYNC["逐项目启动历史同步\nPOST /migration/start?projectId=A\n同上 B, C\n→ CS_START → JOB_FULL → CATCH_UP → VERIFY → READY"]

    SYNC --> SYNC_NOTE["此时 routing-state 仍为 OFF\n所有项目读写全走 Default\nNodeProjectSyncJob 按组内全部 projectIds 写 Heavy"]

    SYNC_NOTE --> DUAL_A["A 到达 READY 后开启双写\nPOST /migration/dual-write?projectId=A\n→ routing-state=DUAL_WRITE\n→ A 双写 Heavy+Default, B/C 仍走 Default"]

    DUAL_A --> ROUTE_A["A 验证通过后切流\nPOST /migration/route?projectId=A\n→ A.phase=ROUTED, routing-state=ROUTED\n→ A 单写 Heavy, B/C 回落 Default"]

    ROUTE_A --> DUAL_B["重复 B 的双写+切流\nPOST /migration/dual-write?projectId=B\n→ routing-state=DUAL_WRITE\nA (isRouted=true) 不受影响"]

    DUAL_B --> ROUTE_B["B 切流 → routing-state=ROUTED"]

    ROUTE_B --> DUAL_C["重复 C 的双写+切流"]

    DUAL_C --> ROUTE_C["C 切流 → routing-state=ROUTED"]

    ROUTE_C --> CLEAN["逐项目清理 Default\nPOST /migration/cleanup?projectId=A\n同上 B, C"]
```
...
| --- | --- | --- | --- | --- | --- |
| 绑定 | OFF | PENDING | PENDING | PENDING | `POST /migration/binding?businessId=biz-ci` |
| A 历史同步 | OFF | CS_START→READY | PENDING | PENDING | `POST /migration/start?projectId=A` |
| A 双写 | DUAL_WRITE | Heavy+Default 双写 | Default | Default | `POST /migration/dual-write?projectId=A` |
| A 切流 | ROUTED | Heavy 单写 | Default | Default | `POST /migration/route?projectId=A` |
| **… 观察期 …** | ROUTED | Heavy 单写 | Default | Default | — |
| B 历史同步 | ROUTED | Heavy 单写 | CS_START→READY | PENDING | `POST /migration/start?projectId=B` |
| B 双写 | DUAL_WRITE | Heavy 单写 (`isRouted=true`) | Heavy+Default 双写 | Default | `POST /migration/dual-write?projectId=B` |
| B 切流 | ROUTED | Heavy 单写 | Heavy 单写 | Default | `POST /migration/route?projectId=B` |

##### 场景 C：组已 ROUTED 后增量（新增项目 D）

A/B/C 已全部 ROUTED 且清理完成，业务需要将新项目 D 加入组。

**前置操作**：当前 `business-routing: biz-ci → [A, B, C]`，Conusl 中 `project-routing` 展开不含 D。必须先改配置再调 API。

```mermaid
flowchart TD
    START["当前: routing-state=ROUTED\nA/B/C ROUTED→Heavy, D不在组内→Default\nbusiness-routing: biz-ci→[A,B,C]"] --> C_CFG

    C_CFG["① Consul 配置变更\nbusiness-routing: biz-ci→[A,B,C,D]\n→ project-routing 展开含 D\n→ D 命中 project-routing 但 isRouted=false\n→ 路由回落 Default（读写安全）"]

    C_CFG --> C_BIND["② POST /migration/binding\nruleName=node, projectId=D\nbusinessId=biz-ci, strategy=JOB_ONLY\n→ 在 mongo_migration_sync_state 创建 D 记录\n→ D.phase=PENDING"]

    C_BIND --> C_BIND_NOTE["D 读写仍走 Default\nNodeProjectSyncJob 按 projectId=D 直写 Heavy"]

    C_BIND_NOTE --> C_SYNC["③ POST /migration/start?projectId=D\n历史同步写 Heavy"]

    C_SYNC --> C_DUAL["④ D READY 后: POST /migration/dual-write?projectId=D\n→ routing-state=DUAL_WRITE\nA/B/C: isRouted=true → Heavy 单写\nD: isProjectInDualWrite=true → 双写"]

    C_DUAL --> C_ROUTE["⑤ POST /migration/route?projectId=D\n→ routing-state=ROUTED\n全组 ROUTED → 全部单写 Heavy"]

    C_ROUTE --> C_CLEAN["⑥ POST /migration/cleanup?projectId=D"]
```

> **ponytail**：场景 C 与场景 B 的唯一区别是**第一步必须先改 Consul `business-routing` 配置**把 D 加进组。不改配置，D 不在 `project-routing` 展开中，路由框架不知道 D 属于这个 Heavy，API 也无法为其绑定。改配置后 D 立刻出现在 `project-routing`，但 `phase=PENDING` → `isRouted=false` → 路由回落 Default，对现有 A/B/C 无任何影响。
...
| API | 关键参数 | 说明 |
| --- | --- | --- |
| `POST /migration/binding` | `ruleName`, `businessId` 或 `projectId`, `strategy` | 初始全组用 `businessId`；增量加项目用 `projectId` |
| `POST /migration/start` | `ruleName`, `projectId` | 每项目独立调用，启动历史同步 |
| `POST /migration/dual-write` | `ruleName`, `projectId` | 每项目独立调用，推 DU state 并改 routing-state |
| `POST /migration/route` | `ruleName`, `projectId` | 每项目独立调用，切流并改 routing-state |
| `POST /migration/cleanup` | `ruleName`, `projectId` | 每项目独立调用，清理 Default 数据 |
| `POST /migration/rollback` | `ruleName`, `projectId` | 每项目独立回滚 |

##### 约束检查清单（全场景通用）

| 约束 | 说明 |
| --- | --- |
| `max-concurrent-dual-write=1` | 组内同时只有一个 `phase=DUAL_WRITE` 的项目（已 ROUTED 不计入） |
| `routing-state` 切换安全 | `isRouted` 隐含 `routing-state != OFF` + `phase >= ROUTED`，OFF 时全部回落 Default |
| 已切流项目不受双写影响 | `isRouted=true` → 永远单写 Heavy，不因其他项目的 `routing-state` 切换而改变 |
| 迁移中禁止向组内新增项目 | C 场景的 D 必须在操作窗口外已存在；`POST /migration/binding` 即声明加入 |
| 回滚安全 | 回滚仅影响目标 `projectId`，组内其他项目不受波及 |

> **ponytail**：Tier-Biz 的运维哲学是「配置以组为单位，操作以项目为粒度」。`business-routing` 定义了共享 Heavy 的一组项目，`POST /migration/binding?businessId=biz-ci` 批量初始化状态记录。但路由决策依赖每项目的 `phase` 字段，`max-concurrent-dual-write=1` 强制串行推进 — 所以无论是全组迁移还是分批迁移，runtime 行为本质相同：逐项目推进。`routing-state` 在 `DUAL_WRITE ↔ ROUTED` 间反复切换，已切流项目靠 `isRouted=true` 保持稳定。
...
> `isProjectInDualWrite`：`routing-state=DUAL_WRITE` + 项目在 `project-routing` 中 + `phase=DUAL_WRITE`
> `isRouted`：`routing-state != OFF` + 项目在 `project-routing` 中 + `phase >= ROUTED`
> **关键**：`routing-state=ROUTED` 但 phase < ROUTED 的项目（Tier-Biz 组内尚未迁移的兄弟项目），路由回落 Default。

| routing-state | isProjectInDualWrite | isRouted | 能提取 projectId | 命中 project-routing | 命中 shard-routing | 读目标 | 写目标 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| OFF | - | - | - | - | - | Default Primary | Default Primary |
| DUAL_WRITE/ROUTED | false | false | 是 | 是 | - | **Default Primary** | **Default Primary** |
| DUAL_WRITE/ROUTED | false | **true** | 是 | 是 | - | 项目实例 Primary | 项目实例 Primary |
| DUAL_WRITE/ROUTED | false | - | 是 | 否 | 是 | 分片实例 Primary | 分片实例 Primary |
| DUAL_WRITE/ROUTED | false | - | 是 | 否 | 否 | Default Primary | Default Primary |
| DUAL_WRITE | true | - | 是 | 是 | - | **Default Primary** | Heavy Primary + Default Primary |
| DUAL_WRITE | true | - | 是 | 否 | 是 | 分片实例 Primary | 分片实例 Primary + Default Primary |
| DUAL_WRITE | true | - | 是 | 否 | 否 | Default Primary | Default Primary |
| DUAL_WRITE/ROUTED | - | - | 否（写） | - | - | - | fail-fast |
| DUAL_WRITE/ROUTED | - | - | 否（读） | - | - | 散发所有实例 | - |

> **Tier-Biz 增量迁移决策流程**：以 `business-routing: biz-ci → [A, B, C]` 为例。
>
> | 时刻 | routing-state | A (phase=ROUTED) | B (phase=DUAL_WRITE) | C (phase=PENDING) |
> | --- | --- | --- | --- | --- |
> | A 迁移完成 | ROUTED | `isRouted=true` → Heavy | `isRouted=false` → **Default** | `isRouted=false` → **Default** |
> | B 开始双写 | DUAL_WRITE | `isRouted=true` → Heavy | `isProjectInDualWrite=true` → 双写 | `isRouted=false` → **Default** |
> | B 迁移完成 | ROUTED | `isRouted=true` → Heavy | `isRouted=true` → Heavy | `isRouted=false` → **Default** |
...
| 方法 | 语义 |
| --- | --- |
| `isRouted(ruleName, projectId)` | `routing-state != OFF` + 项目在 `project-routing` 中 + `phase >= ROUTED` |
| `isProjectRoutedOut(ruleName, projectId)` | 别名，等同 `isRouted` |
| `isProjectInDualWrite(ruleName, projectId)` | `routing-state=DUAL_WRITE` + 项目在 `project-routing` 中 + `phase=DUAL_WRITE` |

> **注意**：`isRouted` / `isProjectRoutedOut` 仅对已完成切流的项目返回 true。Tier-Biz 组内尚未迁移的项目（phase < ROUTED），返回 false，不会触发僵尸写保护、不会路由到 Heavy。
