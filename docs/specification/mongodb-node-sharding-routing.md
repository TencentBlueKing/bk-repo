...
| 双写期（`routing-state=DUAL_WRITE`） | Default Primary（主路径）→ 同步写 Offload Primary，失败则补偿 | Default Primary（数据最完整，含旧实例写入） | 最终一致，补偿延迟上限 5 分钟 |
...
| 2 | G-34 路由就绪（INFRA + M5） | `GET /routing/readiness` |
...
③ 等待 ≥ 30s（覆盖 `@ConfigurationProperties` 原地重绑定的秒级 `routing-state` 生效窗口），或查 Prometheus `bkrepo_mongo_routing_config_version` 确认全集群一致

> **配置生效方式分级（与主方案 §2 / pattern1 §2.10.1 对齐）**：
> - **热加载级（纯内存开关，秒级生效，不重建连接池）**：`routing-state`。`routing-state` 切换**不依赖 `@RefreshScope` 重建 `MongoRoutingRegistry` Bean**；连接池（`MongoTemplate`/`MongoClient`）在启动期一次性建好（普通单例），`routing-state` 经 `@ConfigurationProperties` 原地重绑定后由 `DefaultMongoRoutingRegistry` **live 读取**，下一次 `resolve()` 调用即生效。若误将 `@RefreshScope` 加在该 Bean 上，任何 Consul 刷新都会销毁并重建整个 Registry（连带所有连接池重建），导致切流期间全量连接抖动，**须避免**。
> - **启动级（变更须滚动重启）**：`instances.*.uri`、`collection-prefix`、`routing-type`、`project-routing`/`shard-routing`/`business-routing` 规则元数据。这些变更影响连接池或 `prefixIndex` 初始化，无法热加载，必须滚动重启才会重建连接池。
...
路由决策（双写/切流/读路由）由 **Consul**（`project-routing` + `routing-state`）决定；DB `mongo_migration_sync_state.phase` 仅供 Job / Gate / 面板。
