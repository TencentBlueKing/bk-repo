...
| 模式 | 配置 | 行为 | 适用 |
|---|---|---|---|
| **STANDARD**（默认） | 无 `multi-instance.rules` 或 `routing-enabled=false` | 单 `mongodb.uri` 直连 Default，零额外连接 | 数据量小、无热点项目 |
...
3. **环境可选**：无 rules 或 `routing-enabled=false` 时路由短路，行为与现网等价。
...
- **模式一与模式二启动门禁独立**：oplog 整体迁移（模式一）不要求 node 路由就绪（G-34）；node 项目级迁移（模式二）须 G-34 通过后方可启动迁移编排（§10.5、§14）。
...
| Oplog 实例 | `artifact_oplog_*` 专属 MongoDB 副本集 |
...
| 阶段 | 写入节点 | 读取节点 | 一致性语义 |
| --- | --- | --- | --- |
| 迁移前 / `routing-enabled=false` | Default Primary | Default Secondary | 强一致，与迁移前相同 |
| 双写期（`dual-write=true`） | Default Primary（主路径）→ 异步写 Oplog Primary | Default Secondary（主路径数据最完整） | 最终一致，补偿延迟上限 5 分钟 |
| 切流后（`routing-enabled=true, dual-write=false`） | Oplog Primary | Oplog Secondary | 强一致 |

双写期以 Default 为主路径：Default Primary 写成功即返回成功；Oplog Primary 写失败记录补偿任务异步重试。

**模式二（node 项目级路由）**

| 阶段 | 项目路由命中 | 写入节点 | 读取节点 | 一致性语义 |
| --- | --- | --- | --- | --- |
| `routing-enabled=false` | — | Default Primary | Default Secondary | 强一致，与迁移前相同 |
| `routing-enabled=true, dual-write=false` | 否（未迁出项目） | Default Primary | Default Secondary | 强一致 |
| `routing-enabled=true, dual-write=false` | 是（已迁出项目） | Heavy Primary | Heavy Secondary | 强一致 |
| `routing-enabled=true, dual-write=true` | 是（迁移双写期） | Heavy Primary（主路径）→ 同步写 Default Primary，失败则补偿 | Default Secondary | 最终一致；读走 Default 保证副路径/老 Pod 写入可见，写失败由补偿兜底 |
| 散发查询（`pageBySha256`，无 `projectId`） | — | — | 所有实例 Secondary 并发查询后合并去重 | 最终一致，各实例 Secondary 延迟可能不同 |

双写期以 Heavy 为主路径：Heavy Primary 写失败则直接返回失败；Heavy Primary 写成功后**同步尝试**写 Default Primary，同步失败则记录补偿任务并仍返回成功。**双写期读走 Default Secondary**（与模式一对齐），避免 Heavy 副路径滞后或滚动升级老 Pod 只写 Default 时读不到最新数据；`ROUTED` 切流后再读 Heavy Secondary。

**关键约束**

- 同一文档在任意时刻只属于一个实例（由 `projectId` 路由决定），不存在同一文档跨实例写入。
- 两种模式的双写主路径方向相反（模式一以 Default 为主，模式二以 Heavy 为主），补偿语义一致：主路径写成功即返回，副路径同步写失败则异步补偿，补偿队列未清零时阻断切流。
- 模式二双写是否生效由**全局开关 + 项目状态**共同决定（见 §3.5.1），非 YAML 单一布尔值。
- 散发查询（`pageBySha256`）是弱一致性查询，业务侧已容忍秒级差异。
- `file_reference_*` 按 `sha256` 分表，始终留在 Default 实例，无法随 `node_*` 迁移至 Heavy；`createNode` / `moveNode` / `copyNode` 涉及跨实例写入，`@Transactional` 不能跨副本集，需将 `file_reference` 写入改为补偿模式（详见 3.14 节）。

#### 1.3.1 模式二双写方向设计理由

**核心问题**：为什么模式二以 Heavy 为主路径，而不是像模式一那样以 Default 为主路径？

模式一中 `artifact_oplog_*` 迁移后 **Default 仍然是永久主实例**（Oplog 仅做存储分流），因此双写以 Default 为主路径是自然选择。但模式二中 `node_*` 项目路由的最终目标是**让 Heavy 成为迁出项目的唯一权威数据源**，Default 只是过渡期的"安全网"——最终要清理该项目在 Default 上的全部数据。这一根本差异决定了双写方向必须相反。
...
反向设计（Default 为主）：
  切流前 Default(主) + Heavy(副) → 切流后 Heavy(主) ← 主路径跳变！
```

切流（`DUAL_WRITE` → `ROUTED`）是线上操作，复杂度越低越安全。当前设计下切流只需：① 确认补偿队列清零 ② 关闭 `dual-write` ③ 读路径切到 Heavy Secondary。反向设计还需额外完成"写主路径从 Default 切到 Heavy"——这是一个高风险操作，涉及所有读写语义、`_id` 生成权和失败处理策略的瞬间改变。

##### 理由 2：`_id` 全生命周期一致性

```
当前设计：双写期 _id 由 Heavy 生成 → Default 复用
         切流后 _id 由 Heavy 生成
         全生命周期 _id 同源 ✅

反向设计：双写期 _id 由 Default 生成 → Heavy 复用
         切流后 _id 由 Heavy 生成
         前后 _id 生成源不一致 ⚠️
```

以 Heavy 为主路径，保证切流前后 `_id` 生成策略一致，对账和溯源无歧义。

##### 理由 3：写失败语义——宁可不可用，不可丢数据

| 场景 | 当前设计（Heavy 主） | 反向设计（Default 主） |
|---|---|---|
| 主路径写失败 | 直接返回失败，业务重试 | 直接返回失败，业务重试 |
| | 两侧均无数据，安全 ✅ | 两侧均无数据，安全 ✅ |
| 主路径成功 + 副路径失败 | Heavy 有，Default 缺失 | **Default 有，Heavy 缺失** |
| | 切流后 Heavy 数据完整 ✅ | 切流后 Heavy 丢数据 ❌ |
| | 补偿异步补齐 Default | 补偿补齐 Heavy，但切流前补偿未清零 = 阻断切流 |

**反向设计的关键陷阱**：Default 写成功就返回成功，但 Heavy 上可能没有这条数据。切流到 `ROUTED` 后读 Heavy，会丢失当时已告知业务"写成功"的数据。而当前设计下，Heavy 上一定有所有返回成功的数据（Heavy 失败直接返错），Default 上的缺失只影响双写期的读——而双写期读本来就走 Default Secondary，不受影响。

##### 理由 4：读写分离容错——Heavy 故障时"写停读不停"

```
当前设计（Heavy 主）：
  Heavy 宕机 → 迁出项目写入 fail-fast
               迁出项目读取 Default Secondary 仍可用 ✅
               
反向设计（Default 主）：
  Default 宕机 → 所有项目读写全停 ❌
```

当前设计下 Heavy 故障只影响迁出项目的**写**（且是 fail-fast，不会静默丢数据），**读不受影响**——因为双写期读走 Default Secondary，所有数据在 Default 上都有副本。而 Default 是全局依赖，Default 宕机在任何设计下都是灾难，与双写方向无关。

##### 理由 5：滚动升级老 Pod 写入的天然兼容

```text
━━━━━━━━━ 滚动升级时间轴 ━━━━━━━━━
Pod-1: [新代码] 双写 Heavy(主) + Default(副)
Pod-2: [老代码] 只写 Default ← 无路由能力
Pod-3: [新代码] 双写 Heavy(主) + Default(副)

当前设计：
  老 Pod → Default ✓     新 Pod → Heavy + Default ✓
  所有写入在 Default 上都有副本 ✅
  读走 Default Secondary → 能读到所有数据 ✅

反向设计：
  老 Pod → Default ✓     新 Pod → Default(主) + Heavy(副) ✓
  Default 也有全量数据 ✅
  但 Heavy 缺失老 Pod 写入 ← 切流时补偿队列无法检测这些缺失
```
...
```
当前设计（Heavy 主）：
  补偿方向：Heavy → Default（把缺的数据补到副路径）
  补偿的目标是 Default ← Default 最终要被清理
  补偿未清零 = 阻断切流 ← Default 数据不全，不能安全清理
  
反向设计（Default 主）：
  补偿方向：Default → Heavy（把缺的数据补到主路径的未来）
  补偿的目标是 Heavy ← Heavy 是最终归宿
  补偿未清零 = 数据不全，也不能切流
  但：Default 数据全但不能保证 Heavy 也全（老 Pod 写入不在 Heavy）
```

两种设计的补偿语义在形式上对称，但当前设计下补偿的"源"（Heavy）和"目标"（Default）与最终归宿一致——Heavy 上有的就是最终完整数据，Default 上的是临时副本。反向设计则需要"把 Default 的数据补到 Heavy 上"，切流时 Heavy 的数据完整性依赖补偿队列，而补偿队列无法检测老 Pod 写入的缺失。

##### 理由 7：模式一为何方向相反？

| 维度 | 模式一 | 模式二 |
|---|---|---|
| 迁移后 Default 角色 | **永久主实例**（Oplog 只分流） | **不再承载迁出项目数据**，最终清理 |
| 双写后 Default 是否继续承载写入 | 是（未迁出项目 + 非 oplog 集合） | 是（未迁出项目），迁出项目不再写 Default |
| 主路径切换 | **不存在**——Default 始终是主 | Heavy 始终是主，副路径关闭即可 |
| 回滚难度 | Default 数据最完整，关闭路由即可 | Default 有完整数据，但双写期+切流后增量需反向同步 |

模式一中 Default 是唯一永久主实例，Oplog 只做存储分流，双写以 Default 为主路径是自然推理。模式二中 Heavy 是项目的新家，双写以 Heavy 为主路径同样是自然推理。**两种模式的方向相反，但设计原则一致：以迁移后的最终归宿为主路径。**
...
| **NONE 模式双写 update 历史 doc** | ❌ 找不到 | 不写 | 依具体实现 | Heavy 无历史数据 | ❌ 需转为 SYNC_JOB |

#### 1.3.2 双写期的"写后读"一致性窗口分析

**核心问题**：双写期写 Heavy → 同步写 Default Primary → 返回成功 → 下一次读 Default Secondary → **可能读不到刚写入的数据？**

这个担忧是正确的，但需要注意：**这个窗口不是因为"双写"引入的，而是 MongoDB 读写分离架构固有的**。双写期唯一新增的是 Heavy→Default Primary 的同步延迟（< 5ms），而真正决定窗口大小的 Default Primary→Secondary oplog 复制延迟，双写前后完全相同。
...
Pre-migration（单实例 Default）：
  ...
  T1: Default Primary oplog → Secondary  ← 0-100ms（MongoDB 异步复制）
  T2: 客户端读 Default Secondary
  窗口 = T1 - T0 = Default Primary→Secondary 复制延迟

Dual-write（双写期）：
  T0: 客户端写 Heavy Primary            ← 1-5ms
  T1: 同步写 Default Primary             ← 1-5ms（阻塞，不返回）  
  T2: Default Primary oplog → Secondary  ← 0-100ms（MongoDB 异步复制）
  T3: 返回成功给客户端
  T4: 客户端读 Default Secondary
  窗口 = T4 - T2 = Default Primary→Secondary 复制延迟
  
  Heavy→Default Primary 的同步写入在 T3 之前完成，
  T3 时刻 Default Primary 已有数据 ✅
  T4 时刻 Default Secondary 是否有数据取决于 oplog 复制延迟
```

**双写期的"同步"是关键**——Heavy 成功后**阻塞等待** Default Primary 写入返回，才将最终结果返回客户端。客户端收到"成功"时，Default Primary 上**一定有数据**。不会出现"Heavy 有但 Default Primary 还没有"的异步窗口。

##### 窗口影响量化

```
┌─────────────────────────────────────────────────────────────┐
│ 写入完成 → Default Primary 有数据                              │
│                                                              │
│ ───── Default Primary → Secondary oplog 复制 ─────            │
│                                                              │
│ 在复制完成前，读 Default Secondary 看不到新数据 ← 窗口在此     │
│                                                              │
│ 复制完成后，读 Default Secondary 看到新数据                    │
└─────────────────────────────────────────────────────────────┘

正常情况: 复制延迟 < 100ms → 读不到窗口极短，与迁移前一致
异常情况: 复制延迟 > 1s   → 读不到窗口扩大，但同样是迁移前就存在的问题
```

| 场景 | 迁移前 (Default 单实例) | 双写期 | 是否新增窗口 |
|---|---|---|---|
| **正常复制延迟 (< 100ms)** | 写后立即读可能看不到，100ms 后可见 | 完全相同 | ❌ 非新增 |
| **Secondary 复制延迟大 (> 1s)** | 写后读可能 1s+ 看不到 | 完全相同 | ❌ 非新增 |
| **Heavy 成功 + Default Primary 同步失败** | N/A | 补偿任务异步补齐 → 读 Default Secondary 长期看不到 | ✅ **这是新增窗口！** |
| **并发读在 Heavy→Default 同步期间到达** | N/A | Default Primary 还没收到数据，但 Heavy 已写入 → 读 Secondary 看不到 | ⚠️ 理论存在但窗口 < 5ms |

> **唯一的新增窗口（第三种场景）**：Heavy 写成功 + Default Primary 同步写失败 → 记录补偿任务，返回成功。此时 Heavy 有数据，但 Default 完全不知道这条数据。后续读 Default Secondary **长期看不到**（直到补偿任务消费成功）。此时段内这就是"写后读不到"。但补偿队列有监控：积压 > 阈值 → 告警 → 阻断切流（不允许 DUAL_WRITE → ROUTED）。
...
flowchart LR
    subgraph "迁移前"
        W1["写 Default Primary"] --> R1["读 Default Secondary"]
        R1 -. "复制延迟窗口 < 100ms" .-> V1["可能读不到最新数据"]
    end
    
    subgraph "双写期（正常路径）"
        W2["写 Heavy Primary"] --> W2B["同步写 Default Primary\n（阻塞，< 5ms）"]
        W2B --> R2["读 Default Secondary"]
        R2 -. "复制延迟窗口 < 100ms" .-> V2["可能读不到最新数据"]
    end

    subgraph "双写期（补偿路径）"
        W3["写 Heavy Primary ✅"] --> W3B["写 Default Primary ❌"]
        W3B --> C3["记录补偿任务"]
        C3 --> R3["读 Default Secondary"]
        R3 --> V3["长期读不到 ❌\n直到补偿消费成功"]
    end
...
|---|---|---|---|---|
| **读 Default Secondary（当前）** | 同迁移前 | ✅ 天然可见 | ✅ 不受影响 | 仅改读目标 |
| **读 Heavy Secondary** | 无延迟 ✅ | ❌ 老 Pod 写入不可见 | ❌ 读不可用 | 同左 |
| **读 Heavy Primary** | 无延迟 ✅ | ❌ 老 Pod 写入不可见 | ❌ 读写全停 | 切流需降级为 Secondary |

当前选择以**微小的复制延迟窗口**换取**老 Pod 兼容 + Heavy 故障读可用 + 切流简单**三重收益。这是一个有意识的 trade-off，而非设计疏忽。

##### 业务侧应对建议

| 场景 | 建议 |
|---|---|
| **写后立即读同一文档**（如 createNode 后立即 findById） | 此为应用级"read-your-writes"需求，与 MongoDB 副本集架构无关。业务侧如需强一致性读，应使用 `readPreference=primary` 或缓存写入结果 |
| **对延迟不敏感的业务** | 无需处理，< 100ms 窗口用户无感知 |
| **补偿路径下的读缺失** | 补偿队列监控 + 告警 + 阻断切流确保窗口不扩大 |

> **总结**：双写期"写 Heavy 读 Default Secondary"的读不到窗口，在正常路径下与迁移前完全一致（Default Primary→Secondary 复制延迟，< 100ms）。唯一的新增风险是补偿路径下的数据缺失，但补偿队列监控和切流门禁确保不会被忽略。这是"用已知的、迁移前就存在的 window 换取老 Pod 兼容性"的务实设计。
...
| --- | --- | --- | --- |
| `node_*` | MongoDB 自动 `ObjectId` | `projectId + fullPath` | 同一项目下路径唯一，但中间状态（move/copy）下路径可能瞬态不一致 |
| `artifact_oplog_*` | MongoDB 自动 `ObjectId` | `projectId + sha256` | 追加写日志，按月分表 |
| `file_reference_*` | MongoDB 自动 `ObjectId` | `projectId + repoName + path` | 不迁移，始终在 Default |
...
| 问题 | 说明 |
| --- | --- |
| 部分集合无业务唯一键 | 如 `node_file_ref_compensation` 等集合没有可用的业务唯一键 |
...
各模式的迁移状态（`INIT` / `INITIAL_SYNC` / `DBA_DUMP` / `CATCH_UP` / `VERIFY` / `READY` / `DUAL_WRITE` / `ROUTED` / `CLEANED`）为 **Job 内部工作状态**，由 SYNC_JOB / DBA_DUMP 各自独立管理，不对外暴露。运维通过 Consul 配置（`project-routing` + `dual-write`）来控制迁移节奏，Job 通过校验结果输出日志供运维判断。

> **关键**：`migrationMode` 从 Consul 配置读取（如 `rules.node.migration.mode`），不单独持久化。Job 内部进度（`lastSyncedId`、`resumeToken`、`dbaDumpCompleted` 等）写入 `node_project_sync_state` 集合，重启后断点续传；路由决策（`project-routing`、`dual-write`、`routing-enabled`）仅来自 Consul。
...
**模式一 NONE**：不迁 Default 存量；双写切流后读写仅走 Oplog；Default 存量为待清理僵尸数据，**禁止**跨实例读合并（§2.11）。
...
路由决策（双写/切流/读路由）来自 **Consul**；Job 进度写入 `mongo_migration_sync_state`，**不参与**运行时路由决策。
...
|---|---|---|
| **NONE**（模式一） | 双写补偿队列 | = 0 |
| **NONE**（模式一） | `[dualWriteStartAt, now)` count | Oplog ≥ Default 同区间 |
...
模式一 NONE **不做** Default vs Oplog 全量历史 count 比对。
...
`AbstractMongoDao` 基类会自动匹配 `collection-prefix`，将所有 `artifact_oplog_*` 集合的读写路由到 Oplog 实例（详见 §19.4.2b）。
...
flowchart TD
    ...
    CHECK5{"清理完成?\n磁盘回收确认?"}
    DONE["迁移完成"]
    ROLLBACK["回滚：routing-enabled=false\n回退到主实例"]
...
| 1→2 | 专属实例部署完成、索引创建完成、连接验证通过 | dual-write 配置生效 | 是 |
| 2→3 | 双写期 count 对账通过、补偿队列清零 | 所有历史月份 restore 完成 | 是 |
| 3→4 | 各月 count 一致、最新月份双写对账通过 | routing-enabled=true 生效 | 是 |
...
```mermaid
flowchart TD
    A["写 artifact_oplog\n(insert / update / delete)"] --> B{"routing-enabled?"}
...
| --- | --- | --- | --- | --- |
| 0~1 兼容 | routing-enabled=false | Default Primary | 强一致 | 上层重试 |
| 2 双写 | routing-enabled=true, dual-write=true | Default Primary → 异步 Oplog Primary | 最终一致（补偿≤5min） | 主路径失败→直接返回失败；副路径失败→补偿兜底 |
| 4+ 切流 | routing-enabled=true, dual-write=false | Oplog Primary | 强一致 | 上层重试，禁止降级 Default |

#### 2.5.2 读流程

```mermaid
flowchart TD
    A["读 artifact_oplog"] --> B{"routing-enabled?"}

    B -- "否\n阶段 0~1\n兼容模式" --> C["读 Default Secondary"]
    C --> C1{"读成功?"}
    C1 -- "是" --> C2["返回结果\n✅ 强一致"]
    C1 -- "否" --> C3["返回错误\n上层重试或降级"]
    ...
    D -- "是\n双写期" --> E["读 Default Secondary\n(主路径数据最完整)"]
    E --> E1{"读成功?"}
    E1 -- "是" --> E2["返回结果\n⚠️ 最终一致\n(含双写期所有数据)"]
    E1 -- "否" --> E3["返回错误\nDefault 实例故障\n运维介入"]

    D -- "否\n阶段 4+\n已切流" --> F["读 Oplog Secondary\n(专属实例从库)"]
    F --> G{"读成功?"}
    G -- "是" --> H["返回结果\n✅ 强一致"]
    G -- "否" --> I{"Oplog 实例\n不可用?"}
...
| --- | --- | --- | --- | --- |
| 0~1 兼容 | routing-enabled=false | Default Secondary | 强一致 | Default 故障→业务中断 |
| 2 双写 | routing-enabled=true, dual-write=true | Default Secondary | 最终一致（完整） | Default 故障→业务中断；Oplog 故障不影响读 |
| 4+ 切流 | routing-enabled=true, dual-write=false | Oplog Secondary | 强一致 | Oplog 故障→fail-fast，禁止 fallback |

#### 2.5.3 补偿原则

| 场景 | 处理 | 原因 |
| --- | --- | --- |
| 主实例写失败 | 直接返回失败，不写专属实例，不记录补偿 | 主实例是双写期的权威数据源 |
| 主实例写成功、专属实例写失败 | 记录补偿任务，业务返回成功 | 不影响业务；补偿任务消费后数据最终一致 |
| 补偿任务重试成功 | 标记补偿完成，清除任务 | 正常恢复路径 |
| 补偿任务重试达到上限 | 告警人工介入，禁止切流 | 防止数据不一致状态下切换路由 |
| 补偿队列未清零 | 阻断阶段 2→3 流转 | 确保专属实例数据完整后才迁移历史数据 |
...
flowchart TD
    ...
    B -- "阶段 1\nrouting-enabled=false" --> C1["无需操作\n行为与旧版本一致"]
    C1 --> DONE

    B -- "阶段 2 双写期" --> C2["routing-enabled=false\ndual-write=false\n主实例数据完整"]
    ...
    B -- "阶段 4 已切流\n主实例未清理" --> C4["routing-enabled=false\n主实例数据完整"]
    ...
    C5C --> C5D["routing-enabled=false"]
    C5D --> DONE

    B -- "阶段 5 清理已完成" --> C6["专属实例→主实例\n全量反向 mongodump/restore"]
    C6 --> C6A["全量对账"]
    C6A --> C6B["routing-enabled=false"]
    C6B --> DONE
...
flowchart TD
    ...
    B -- "阶段 4 已切流\n主实例数据完整" --> F1["routing-enabled=false\n回退到主实例"]
    F1 --> F2["业务恢复\n等待专属实例修复"]
...
| 场景 | 自动恢复 | 处理方式 | 业务影响 |
| --- | --- | --- | --- |
| routing-enabled 误设为 true（专属实例未就绪） | 否 | 立即回退配置为 false，验证业务恢复 | oplog 读写失败 |
| dual-write 误关闭（迁移未完成） | 否 | 重新开启 dual-write，补偿期间差异 | 专属实例数据缺失 |
...
│  │   rules.artifact-oplog.routing-enabled                   │ │
│  │   rules.artifact-oplog.dual-write                       │ │
│  │   config-version / min-config-version                    │ │
│  └─────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────┘
```

> **说明**：模式一为 `routing-type: NONE`（整体迁移），不涉及 `project-routing`/`shard-routing`。`routing-enabled`/`dual-write` 直接在 Consul 中修改即可（变更频率极低，每条规则仅需切换 1~2 次）。
      ...
      multi-instance:
        ...
        rules:
          artifact-oplog:
            routing-type: none                           # 整体迁移，不需要路由键
            collection-prefix: "artifact_oplog_"         # 匹配此前缀的集合自动路由
            routing-enabled: false                       # 阶段 1 不开启路由
            dual-write: false                            # 阶段 1 不开启双写
            migration:
...
| `routing-enabled` / `dual-write` | **热加载** | `@RefreshScope` 秒级生效 |
...
- [ ] Oplog 专属实例部署完成、副本集 ≥ 3 健康节点
- [ ] Oplog 实例索引已创建（与 Default 一致，见 §8）
- [ ] 从 Pod 内 `mongo --host oplog-primary` 连接验证通过
- [ ] 所有 Pod 已完成滚动重启、`config-version` ≥ 1

**操作**：在 Consul 中修改 `dual-write = true`。
...
spring.data.mongodb.multi-instance:
  ...
  rules.artifact-oplog.dual-write: true
```
...
**操作**：配置不变（`dual-write=true`, `routing-enabled=false`），执行 `mongodump` + `mongorestore`。
...
**验证**：每完成一个月份，对比 Default 和 Oplog 的 `count()`。
...
- [ ] 各月 `count()` 一致（Default == Oplog）
- [ ] 最新月份双写对账通过、补偿队列清零
- [ ] 稳定双写 ≥ 1 天，无异常告警

**操作**：在 Consul 中修改 `routing-enabled = true`, `dual-write = false`。
...
spring.data.mongodb.multi-instance:
  ...
  rules.artifact-oplog:
    routing-enabled: true      # 切流：读写全走 Oplog 实例
    dual-write: false          # 关闭双写
```
...
- [ ] 切流后稳定运行 1~2 天
- [ ] 业务无异常告警
- [ ] 监控指标正常（Oplog 实例 I/O、连接数在阈值内）

**操作**：配置不变（`routing-enabled=true`），执行 Default 实例数据清理（按月份逐月删除）。
...
| 校验项 | 规则 | 不通过行为 |
|--------|------|-----------|
| NONE 模式 + routing-enabled → instances 非空 | `routingEnabled && routingType==NONE → instances.isNotEmpty()` | 启动失败 |
...
> **注意**：回滚后 Oplog 实例上的数据不再写入新数据，若之后需要重新迁移，需从阶段 0 重新开始（因为回滚期间 Default 和 Oplog 的数据已分叉）。

### 2.11 ROUTED 后读写语义（禁止 fallback Default）

切流后（`routing-enabled=true`，`dual-write=false`），`artifact_oplog` 读写**仅走 Oplog 实例**：

| 操作 | 目标 | 失败处理 |
| --- | --- | --- |
| 写 | Oplog Primary | fail-fast，**禁止**写 Default |
| 读 | Oplog Secondary | fail-fast，**禁止**读 Default 僵尸存量 |

Default 上 oplog 存量为待清理数据（§1.6.1 NONE），读 Default 可能返回过期/重复审计记录。应急仅运维显式 `routing-enabled=false` 或回滚 API（§2.8）。

**模式一默认策略 `NONE`**：滚动上线 → 双写（Default 主路径）→ 补偿清零 → ROUTED → 运维触发清理 Default 存量月集合；**不迁移** Default 历史到 Oplog。
...
- 删除 `rules.node` 条目或设置 `routing-type` 为空等价于 `routing-enabled=false`，全部降级到 Default。

#### 3.5.1 双写决策（纯 Consul 驱动）

双写是否生效由 Consul 配置直接决定：**项目在 `project-routing` 中 + `dual-write=true`** 即为双写模式。

```kotlin
fun isDualWriteEnabled(projectId: String): Boolean {
    if (!properties.routingEnabled || !properties.globalDualWrite) return false
    return properties.projectRouting.containsKey(projectId)
}
```

| 项目在 `project-routing` | 全局 `dual-write` | 实际写行为 | 实际读行为（已路由项目） |
| --- | --- | --- | --- |
| 否 | - | 单写 Default | Default Secondary |
| 是 | `true` | Heavy Primary + Default Primary（副路径） | Default Secondary |
| 是 | `false` | **单写 Heavy**（已切流） | Heavy Secondary |

**说明**：
- 双写期项目同时在 `project-routing` 中且 `dual-write=true`，由运维手动维护。
- 切流时运维将 `dual-write` 设为 `false` 但**保留**项目在 `project-routing` 中——此后该项目写入只走 Heavy。
- 回滚时从 `project-routing` 中移除项目即可恢复单写 Default。
- 路由决策（双写/切流/读路由）仅基于 Consul 配置；`node_project_sync_state` 仅记录 Job 迁移进度，不参与路由决策。

**并行迁移约束**

| 规则 | 说明 |
| --- | --- |
| 同一时刻仅一个项目处于双写 | 运维约定，`project-routing` 中同时最多有一个项目搭配 `dual-write=true` |
| 已切流项目不受全局 `dual-write` 影响 | 项目在 `project-routing` 但 `dual-write=false` → 单写 Heavy |
| 所有在途项目切流后关闭全局 `dual-write` | 降低配置误操作面 |

```yaml
spring.data.mongodb.multi-instance.rules.node:
  dual-write: false          # 全局开关；project-routing 中有项目时才打开
  migration:
...
#### 3.6.1 路由总流程（含 dual-write）

```mermaid
flowchart TD
    A["NodeDao 操作"] --> A1{"routing-enabled?"}
    A1 -- 否 --> DEFAULT_RW{"读 or 写?"}
    DEFAULT_RW -- 读 --> DEFAULT_R["Default Secondary"]
    ...
    READ_DW -- 是 --> DEFAULT_READ["Default Secondary\n双写期读主路径副本"]
    READ_DW -- 否 --> READ["目标实例 Secondary"]
...
> `dual-write` 列表示**项目在 `project-routing` 中且全局 `dual-write=true`**（见 §3.5.1）。

| routing-enabled | 项目双写生效 | 能提取 projectId | 命中 project-routing | 命中 shard-routing | 读目标 | 写目标 |
| --- | --- | --- | --- | --- | --- | --- |
| false | - | - | - | - | Default Secondary | Default Primary |
| true | false | 是 | 是 | - | 项目实例 Secondary | 项目实例 Primary |
| true | false | 是 | 否 | 是 | 分片实例 Secondary | 分片实例 Primary |
| true | false | 是 | 否 | 否 | Default Secondary | Default Primary |
| true | true | 是 | 是 | - | **Default Secondary** | Heavy Primary + Default Primary |
| true | true | 是 | 否 | 是 | 分片实例 Secondary | 分片实例 Primary + Default Primary |
| true | true | 是 | 否 | 否 | Default Secondary | Default Primary |
| true | - | 否（写） | - | - | - | fail-fast |
| true | - | 否（读） | - | - | 散发所有实例 | - |
...
| --- | --- | --- | --- | --- | --- |
| 兼容 | routing-enabled=false | Default Primary | MongoDB 自动生成 | 强一致 | 上层重试 |
...
双写期读走 **Default Secondary**（与模式一对齐），确保副路径滞后或滚动升级老 Pod 只写 Default 时能读到最新数据。

```mermaid
flowchart TD
    A["读 node"] --> B{"routing-enabled?"}
    ...
    DW -- "是\n双写期" --> READ_DEFAULT["读 Default Secondary\n(主路径数据最完整)"]
    READ_DEFAULT --> RD1{"读成功?"}
    RD1 -- "是" --> RD2["返回结果\n⚠️ 最终一致\n(覆盖双写期所有数据)"]
    RD1 -- "否" --> RD3["返回错误\nDefault 实例故障\n运维介入"]

    DW -- "否\n(非双写期)" --> READ_TARGET["读目标实例 Secondary"]
    READ_TARGET --> RT1{"读成功?"}
    RT1 -- "是" --> RT2["返回结果\n✅ 强一致"]
    RT1 -- "否" --> RT3{"目标实例\n不可用?"}
...
| --- | --- | --- | --- | --- |
| 兼容 | routing-enabled=false | Default Secondary | 强一致 | Default 故障→业务中断 |
| 双写期 | project-routing 命中, DUAL_WRITE | **Default Secondary** | 最终一致(完整) | Default 故障→业务中断；Heavy 故障不影响读 |
| 切流后 | project-routing 命中, ROUTED | Heavy Secondary | 强一致 | Heavy 故障→fail-fast, 禁止 fallback Default |
| 未路由 | 未命中任何路由 | Default Secondary | 强一致 | Default 故障→业务中断 |
| 无 projectId | 无法提取 | 散发所有实例 Secondary | 最终一致 | 见 §3.7 |
...
>   → "heavy1" → {"projectA", "projectB"}   ← 配置中记录：这两个项目迁到了 heavy1
>   → "heavy2" → {"projectC"}               ← 配置中记录：这个项目迁到了 heavy2
>
...
>
> 每新增一个迁出项目，路由配置更新后，散发读的 projectId 过滤值自动变更（读取配置，无需改代码）。
...
flowchart TD
    ...
    B1 --> C["Default Secondary<br/>追加 projectId NOT IN 已迁出项目"]
    B1 --> D["Instance-1 Secondary<br/>追加 projectId IN 该实例项目集"]
    B1 --> E["Instance-N Secondary<br/>追加 projectId IN 该实例项目集"]
...
#### 3.7.2 `projectId` 过滤退化与白名单策略

Default 侧 Job / 散发查询默认使用 `projectId NOT IN [已迁出项目]`。已迁出项目数增多时，优化器可能退化或执行计划不稳定。

| 已迁出项目数 | 策略 |
| --- | --- |
| ≤ 20 | `projectId NOT IN [已迁出列表]`（当前默认） |
| > 20 | 切换为 **`projectId IN [未迁出白名单]`**（从配置中心或注册表维护） |
| 任意 | 监控 `explain`；`COLLSCAN` 持续 > 5min 告警 |

```kotlin
fun buildDefaultProjectFilter(routedOut: Set<String>, remaining: Set<String>): Criteria {
    return if (routedOut.size <= config.notInThreshold) {
        Criteria.where("projectId").nin(routedOut)
    } else {
        Criteria.where("projectId").`in`(remaining)  // 白名单，避免大 NOT IN
    }
}
```

`remaining` 由 `project-routing` 反推：Default 承载项目 = 全量已知项目 − 已迁出项目；新增项目默认在 Default，直至写入 `project-routing`。
...
```mermaid
flowchart TD
    A["MongoDbBatchJob 启动"] --> A1{"routing-enabled?"}
...
stateDiagram-v2
    ...
    READY --> DUAL_WRITE : 运维在 Consul 中配置 project-routing + dual-write=true
...
| ROUTED ~ CLEANUP | 僵尸副本，不再更新 | `fallback-before-cleanup` 应急读；对账抽样 | 任何 Job/脚本对 Default 上迁出项目做 delete/update |
...
flowchart TD
    ...
    S3["阶段 3 dual-write=true\n项目在 project-routing 中"]
    S3_CHECK{"100% 新 Pod?\n补偿队列清零?\n对账通过?"}
    S4["阶段 4 dual-write=false\n项目单写 Heavy"]
    S4_CHECK{"稳定运行 1~2 个\nJob 周期?\nJob 扫描和写回正确?"}
    S5["阶段 5 清理 Default\n目标项目数据"]
    S5_CHECK{"清理完成?\n磁盘回收确认?"}
    DONE["迁移完成"]

    ROLLBACK_S3["回滚：routing-enabled=false\ndual-write=false\n回退到 Default"]
    ROLLBACK_S4["回滚：重新开启\ndual-write=true"]
...
> **硬门禁**：为某项目开启 `DUAL_WRITE` 前，必须 **100% Pod 已部署路由代码且 `routing-enabled=true`**。禁止在新旧 Pod 并存时进入双写——老 Pod 只写 Default 且双写期 CATCH_UP 已暂停（§3.15.7），Heavy 将缺失老 Pod 写入；双写期读已改走 Default Secondary（§1.3），但**仍须消灭老 Pod 写入窗口**。

| Pod 类型 | 读（迁出项目，双写期） | 写 | 说明 |
| --- | --- | --- | --- |
| 老 Pod（无路由代码） | — | — | **双写前必须摘流/滚动完毕**，不得与双写并存 |
| 新 Pod（项目 `status=DUAL_WRITE`） | **Default Secondary** | Heavy Primary + Default Primary | 读走 Default，与 §1.3 一致 |
| 新 Pod（项目未命中路由） | Default Secondary | Default Primary | 未迁出项目行为不变 |
...
flowchart TD
    ...
    B -- "阶段 1\nrouting-enabled=false" --> R1["无需操作\n停止 SyncJob 即可"]
    R1 --> DONE

    B -- "阶段 2 同步中" --> R2["停止 NodeProjectSyncJob\n清理 Heavy 已同步数据（可选）"]
    R2 --> DONE

    B -- "阶段 3 dual-write=true" --> R3_BRANCH{"migrationMode?"}
    R3_BRANCH -- "SYNC_JOB/DBA_DUMP\n（Heavy 有历史数据）" --> R3A["routing-enabled=false\ndual-write=false\nDefault 数据完整\n业务立即恢复"]
    R3A --> DONE
    R3_BRANCH -- "NONE\n（Heavy 无历史数据）" --> R3B["routing-enabled=false\ndual-write=false\nDefault 数据完整"]
    R3B --> R3B1["Heavy 中仅有双写期新数据\n回滚后不再写入 Heavy"]
    R3B1 --> DONE

    B -- "阶段 4 单写 Heavy\nDefault 未清理" --> R4_BRANCH{"migrationMode?"}
    R4_BRANCH -- "SYNC_JOB/DBA_DUMP\n（Heavy 有历史 + 新数据）" --> R4{"切流期间\nHeavy 有新写入?"}
    R4 -- 否 --> R4A["routing-enabled=false\nDefault 数据完整"]
    R4A --> DONE
    R4 -- 是 --> R4B["Heavy → Default\n补回切流期间增量"]
    R4B --> R4C["对账通过"]
    R4C --> R4D["routing-enabled=false"]
    R4D --> DONE
    R4_BRANCH -- "NONE\n（Heavy 仅有双写期新数据）" --> R4N{"切流期间\nHeavy 有新写入?"}
    R4N -- 否 --> R4NA["routing-enabled=false\nDefault 数据完整"]
    R4NA --> DONE
    R4N -- 是 --> R4NB["Heavy → Default\n补回双写期增量"]
    R4NB --> R4NC["对账通过（仅双写期数据）"]
    R4NC --> R4ND["routing-enabled=false"]
    R4ND --> DONE
    ...
    R5B --> R5C["对账通过"]
    R5C --> R5D["routing-enabled=false"]
    R5D --> DONE

    B -- "阶段 5 清理完成" --> R6["Heavy → Default\n全量反向同步"]
    R6 --> R6A["全量对账"]
    R6A --> R6B["routing-enabled=false"]
    R6B --> DONE
...
| 双写期 | 是 | 否 | 是 | routing-enabled=false | 秒级 |
...
| 场景 | 自动恢复 | 处理 | 业务影响 |
| --- | --- | --- | --- |
| routing-enabled 误设 true（实例未就绪） | 否 | 立即回退配置为 false | 目标项目读写失败 |
| routing-enabled 误设 false（已切流） | 否 | 恢复为 true；若 Default 已清理则数据缺失 | 读写走 Default（可能数据不完整） |
...
| **Heavy 磁盘使用率 ≥ 85%** | 否 | 拒绝新写入，触发写保护 `migrationGate.close()`（§24.24 E-23） | Heavy 实例写不可用 |
...
| **补偿任务入队失败（MongoDB 不可写）** | 否 | 写本地文件兜底 + P0 告警；补偿恢复扫描器定期重试入队（§24.16 E-15） | Heavy/Default 永久不一致风险 |
...
| Heavy 磁盘使用率 ≥ 85% | 否 | 触发写保护，拒绝新写入（§24.24 E-23）；加速已有项目清理 | Heavy 写不可用 |

### 3.13 后续大项目快速迁移 SOP

1. 统计确认大项目 `projectId` 和所在分片编号（`HashShardingUtils.shardingSequenceFor`）。
2. 配置新增目标实例，`routing-enabled=false` 或暂不加入 `project-routing`。
3. 启动 `NodeProjectSyncJob(projectId, targetInstance)`，等待 `READY`。
4. 开启 `project-routing` 和 `dual-write=true`，滚动发布或动态刷新配置。
5. 对账通过、补偿任务清零后，关闭 `dual-write`。
6. 观察 1~2 个 Job 周期，确认 Job 扫描条件和写回路由正确。
7. 满足 `CLEANUP_READY` 条件后，分批清理 Default 上该项目数据。

### 3.14 file_reference 跨实例一致性

#### 3.14.1 问题

`file_reference_*` 按 `sha256` 分表（`@ShardingKey` 为 `sha256`），同一文件可被多个项目的 `node` 引用，**无法按 `projectId` 随 `node_*` 迁移到 Heavy 实例**，始终留在 Default 实例。

`createNode` / `moveNode` / `copyNode` 带 `@Transactional`，方法体内经 `doCreate` 同时写入两个集合：

```text
nodeDao.insert(node)              → 分库后写 Heavy 实例
fileReferenceService.increment()  → 始终写 Default 实例（file_reference_*）
```

**MongoDB 事务不能跨副本集**。`@Transactional` 绑定的 Session 仅属于一个实例（Heavy），
`fileReferenceDao` 操作的是 Default 实例的独立 MongoTemplate，完全在事务外执行。

分库后失效场景：

| 场景 | 结果 | 影响 |
| --- | --- | --- |
| `node` 写成功，事务因其他原因回滚 | Heavy 端 `node` 回滚，Default 端 `file_reference` 已 +1 不回滚 | 引用计数泄漏，文件永不被 GC 清理 |
| `node` 写成功，`file_reference` 写失败 | `node` 已落库，引用计数未增加 | 引用计数偏低，文件可能被 GC 提前错误清理 |

> **不受影响的路径**：`DeletedNodeCleanupJob` 的物理删除已是分步补偿模式（`remove` → `decrement`，失败则 `increment` 回滚），不依赖 MongoDB 事务，分库后行为不变。

#### 3.14.2 改造方案

将 `doCreate` 中对 `file_reference` 的**同步写入**改为**补偿写入**，与删除路径的处理模式对齐：

```mermaid
flowchart TD
    A["doCreate"] --> B["nodeDao.insert(node)"]
    B --> C{"插入成功?"}
    C -- 否 --> D["抛出异常\n不写 file_reference\n不记补偿"]
    C -- 是 --> E["写入补偿任务\nFileRefIncrementTask(sha256, credentialsKey, +1)"]
    E --> F["返回成功"]
    E -. 补偿调度器异步消费 .-> G["fileReferenceService.increment\nupsert，天然幂等"]
    G --> H{"成功?"}
    H -- 是 --> I["标记补偿完成"]
    H -- "失败，重试 < 上限" --> G
    H -- "失败，重试 >= 上限" --> J["告警，人工介入"]
```

关键约束：
- `nodeDao.insert` 成功是写 `file_reference` 的前提，与当前语义一致。
- `fileReferenceService.increment` 内部已是 upsert + `DuplicateKeyException` 重试，补偿重试天然幂等。
- `@Transactional` 边界缩减到仅覆盖 `nodeDao.insert` 和其他同实例操作，不再跨实例。
- `moveNode` / `copyNode` 通过 `doCreate` 间接调用，改造后同样适用。

#### 3.14.3 补偿任务表

新增集合 `node_file_ref_compensation`（存于 Default 实例），字段：
...
调度器复用 node 双写补偿的现有框架，配置独立的重试上限和告警阈值。

#### 3.14.4 异常场景

| 场景 | 自动恢复 | 处理 | 影响 |
| --- | --- | --- | --- |
| `node` 插入成功，补偿任务写失败 | 否 | 告警；可通过扫描 `node` 与 `file_reference` count 差异定位并手动补写 | 引用计数短暂缺失 |
| 补偿调度器宕机 | 是 | 重启后从补偿表恢复，继续消费 | 补偿延迟 |
| 补偿重试达上限 | 否 | 告警人工排查（sha256 引用计数偏低，影响 GC 判断） | 数据不一致风险 |
| 重复补偿（幂等验证） | 是 | `increment` 是 upsert inc，多次执行无副作用 | 无 |
| 补偿队列积压 > 阈值 | 否 | 告警，排查 Default 实例连接或写入异常 | 引用计数延迟更新 |

#### 3.14.5 file_reference 补偿延迟与 GC 安全窗口

**核心风险**：§3.14.2 将 `file_reference.increment` 改为异步补偿写入后，存在一个**补偿延迟窗口**，
可能触发文件的错误 GC 清理：

```text
T1: doCreate → nodeDao.insert(node) 成功（Heavy）
T2: 写入 FileRefIncrementTask 补偿任务
T3: GC Job 扫描 file_reference_*，发现 sha256 的 refCount 仍为旧值（补偿尚未消费）
T4: GC 逻辑判定 refCount 不足以保留文件，删除底层存储文件
T5: 补偿任务消费后 increment 成功 — 但文件已被删除！
```

**解决方案：迁移期 GC 暂停 + 正常运营期三层防护**

> **迁移期（主防线）**：任何项目处于迁移过程中（INITIAL_SYNC ~ CLEANUP），`file_reference` GC **全局暂停**（由 §3.18.2 的 `migration.project-locks.freeze-gc` 控制）。这是最安全可靠的手段——单项目迁移周期通常数天，孤儿文件积压可忽略不计。
>
> 以下三层防护作为**正常运营期**（无迁移项目或全部 CLEANED 后）的防御纵深，确保异步补偿模式下日常运营中不会误删文件：

```mermaid
flowchart TD
    L0["第〇层（迁移期）：GC 全局暂停\nmigration.project-locks.freeze-gc=true"] --> L1["第一层：GC 安全窗口\n文件创建后 T 时间内跳过 GC\n（默认 T = 5 分钟）"]
    L1 --> L2["第二层：补偿优先级提升\nfile_ref 补偿任务使用独立的高优先级队列\n确保补偿在安全窗口内完成"]
    L2 --> L3["第三层：GC 前二次确认\n执行物理删除前重新查询 refCount\n若与 GC 判定时不一致则跳过"]

    style L0 fill:#fce4e4,stroke:#d93025
...
在 GC Job 的删除判定逻辑中增加"安全窗口"检查：

```kotlin
// file GC 判定逻辑
fun shouldGc(file: FileReference): Boolean {
    val now = Instant.now()
    val safeWindow = Duration.ofMinutes(5)  // 可配置

    // 文件创建后 5 分钟内不触发 GC
    if (file.createdDate.plus(safeWindow).isAfter(now)) {
        return false
    }

    // 最近一次 refCount 变更后 5 分钟内不触发 GC
    if (file.lastRefCountUpdate?.plus(safeWindow)?.isAfter(now) == true) {
        return false
    }
...
`file_reference` 的补偿任务使用**独立的高优先级线程池**，与普通数据补偿分离：

```yaml
compensation:
  file-ref:
    thread-pool-size: 4      # 独立线程池，不被普通补偿阻塞
    max-retry: 5
    retry-interval: 500ms    # 首次重试 500ms，确保快速追平
    degradation-threshold: 1000  # 积压超此阈值触发降级告警
```

**层三：GC 前二次确认**

物理删除文件前，**重新查询一次** `file_reference_*` 确认 `refCount`：

```kotlin
fun executeGc(file: FileReference) {
    // 二次确认：重新查询当前 refCount
    val current = fileReferenceDao.findBySha256(file.sha256)
    if (current != null && current.refCount > 0) {
        log.warn("GC aborted: refCount changed after decision. sha256={}, old={}, new={}",
            file.sha256, file.refCount, current.refCount)
        return
    }
    // 安全删除
    storageService.delete(file.sha256)
}
```

| 场景 | 防护层 | 说明 |
| --- | --- | --- |
| **迁移进行中（任一项目 freeze-gc=true）** | **第〇层（主防线）** | **GC 全局暂停，所有 sha256 跳过 GC 判定** |
| 补偿延迟 < 5 分钟 | 层一 | 安全窗口覆盖，文件不会被 GC 扫描 |
| 补偿延迟 > 5 分钟 | 层二 + 层三 | 高优先级队列加速消费；即使超时，二次确认也会拦截 |
| 补偿队列完全阻塞 | 层三 | 二次确认为最后兜底 |
| `lastRefCountUpdate` 未维护 | 层一（降级） | 回退到仅使用 `createdDate` 安全窗口 |

> **配置项 `file.reference.gc-safe-window-minutes`**：默认 5 分钟。应根据补偿队列 P99 消费延迟的 3 倍来设定。
...
| 维度 | 模式一（Oplog 整体迁移） | 模式二（Node 项目级路由） |
| --- | --- | --- |
| 主路径 | Default Primary | Heavy Primary |
| 副路径 | Oplog Primary | Default Primary |
...
§1.3 定义了数据一致性模型，§3.14 解决了 `file_reference` 跨实例问题，§3.15 补全了 Update/Delete 双写处理。但缺少**系统化的端到端一致性保障体系**——包括自动对账、数据自愈、一致性验证工具和持续监控。
...
flowchart TD
    ...
    style L2 fill:#e6f4ea,stroke:#34a853
    style L3 fill:#fffbe6,stroke:#d4a017
    style L4 fill:#fce4e4,stroke:#d93025
```
...
| 第三层：定期对账 | 自动对账 Job（每小时/每天） | 全阶段 | 分钟~小时 | 检测 count 差异、内容差异 |
| 第四层：数据自愈 | 自动修复 + 人工审批 | 全阶段 | 小时级 | 修复已确认的差异 |
...
#### 3.17.4 第三层：定期对账（自动对账 Job）

**3.17.4.1 对账策略**

| 对账类型 | 触发频率 | 适用阶段 | 检测粒度 |
| --- | --- | --- | --- |
| 快速 count 对账 | 每小时 | 双写期 + 切流后 | 集合级数量差异 |
| `_id` 范围分段 count | 每天凌晨 | 双写期 + 切流后 | 定位差异区间 |
| 关键字段 checksum | 每天 | 双写期 + 切流后 | 字段级内容差异 |
| 全量 `_id` 扫描 | 每周 | 切流后稳定期 | 精确到单文档 |

**3.17.4.2 `_id` 范围分段 count**

比全局 `count()` 更能定位差异区间：

```kotlin
fun segmentedCount(template: MongoTemplate, collectionName: String, segmentCount: Int = 100): Map<String, Long> {
    val minId = template.findOne(Query().with(Sort.by("_id", ASC)).limit(1), collectionName)?._id
    val maxId = template.findOne(Query().with(Sort.by("_id", DESC)).limit(1), collectionName)?._id
    val segments = splitRange(minId, maxId, segmentCount)

    return segments.associate { segment ->
        val count = template.count(Query.query(Criteria.where("_id").gte(segment.start).lt(segment.end)), collectionName)
        "${segment.start}~${segment.end}" to count
    }
}

fun compareSegmentedCount(primary: Map<String, Long>, secondary: Map<String, Long>): List<String> {
    return primary.filter { (range, count) -> count != secondary[range] }.keys.toList()
}
```

**3.17.4.3 关键字段 checksum**

对关键字段做聚合校验，检测内容级差异：

```javascript
// MongoDB 聚合管道：按 projectId 分组，计算关键字段的 checksum
db.node_188.aggregate([
  { $match: { projectId: "projectA" } },
  { $group: {
      _id: "$projectId",
      count: { $sum: 1 },
      sha256_checksum: { $sum: { $toHash: "$sha256" } },
      deleted_count: { $sum: { $cond: ["$deleted", 1, 0] } },
      lastModified_max: { $max: "$lastModifiedDate" }
  }}
])
```

比较两侧聚合结果，若不一致则精确定位差异维度（数量 / 哈希 / 删除状态 / 时间戳）。
...
```kotlin
fun fullIdScan(primary: MongoTemplate, secondary: MongoTemplate, collectionName: String): ScanResult {
    val primaryIds = primary.find(Query().with(Sort.by("_id", ASC)), collectionName)
        .map { it._id as String }.toSet()
    val secondaryIds = secondary.find(Query().with(Sort.by("_id", ASC)), collectionName)
        .map { it._id as String }.toSet()

    return ScanResult(
        onlyInPrimary = primaryIds - secondaryIds,   // 副路径缺失
        onlyInSecondary = secondaryIds - primaryIds, // 主路径缺失（异常）
        matchCount = primaryIds.intersect(secondaryIds).size
    )
}
```

> 注意：全量 `_id` 扫描对大集合（1 亿+文档）开销较大，应在低峰期执行，且仅用于切流后稳定期。

#### 3.17.5 第四层：数据自愈

**3.17.5.1 自动修复策略**

| 不一致类型 | 自动修复策略 | 修复来源 | 审批要求 |
| --- | --- | --- | --- |
| 副路径文档缺失（onlyInPrimary） | 从主路径 `clone` 文档写入副路径 | 主路径数据 | 自动（无需审批） |
| 副路径文档多余（onlyInSecondary） | 从副路径删除多余文档 | 副路径数据 | ⚠️ 需人工审批（可能是合法数据） |
| 关键字段不一致 | 用主路径数据覆盖副路径 | 主路径数据 | 自动（双写期）；需审批（切流后） |
| `_id` 不一致 | 告警，人工处理（不应出现） | — | 仅人工 |

**3.17.5.2 自愈流程**

```mermaid
flowchart TD
    A["定期对账 Job 发现差异"] --> B{"差异类型?"}
    B -- "文档缺失" --> C["从主路径 clone 到副路径"]
    C --> D["校验写入成功"]
    D --> E["记录自愈日志"]

    B -- "文档多余" --> F["记录到人工审批队列\nnode_reconciliation_review"]
    F --> G["人工确认后执行删除"]
    ...
    style F fill:#fffbe6,stroke:#d4a017
    style J fill:#fce4e4,stroke:#d93025
```

**3.17.5.3 自愈操作审计表**

新增集合 `node_reconciliation_log`（存于 Default 实例），记录所有自愈操作：
...
```text
{
  ...
  collectionName,       // 集合名
  docId,                // 文档 _id
  type,                 // MISSING / EXTRA / FIELD_MISMATCH / ID_MISMATCH
  action,               // CLONE / DELETE / OVERWRITE / MANUAL
  beforeSnapshot,       // 修复前快照（JSON）
  afterSnapshot,        // 修复后快照（JSON）
  source,               // PRIMARY / SECONDARY / MANUAL
  operator,             // system / 人工操作人
  createdAt,
  status                // PENDING_REVIEW / APPROVED / EXECUTED / ROLLBACK
}
```
...
class ReconciliationController {
    ...
    /** 快速对账：count + 分段 count */
    @PostMapping("/quick")
    fun quickReconciliation(@RequestParam projectId: String): ReconciliationResult

    /** 深度对账：count + checksum + 抽样字段比对 */
    @PostMapping("/deep")
    fun deepReconciliation(
        @RequestParam projectId: String,
        @RequestParam collectionNames: List<String>
    ...
    fun autoHeal(@RequestParam projectId: String): HealResult

    /** 人工审批队列 */
    @GetMapping("/review-queue")
    fun reviewQueue(@RequestParam projectId: String): List<ReconciliationLog>

    /** 审批通过并执行 */
    @PostMapping("/approve/{logId}")
    fun approveAndExecute(@PathVariable logId: String): HealResult
}
```

> 注意：对账 API 需要配置**权限控制**，仅运维和管理员角色可访问。

#### 3.17.7 持续监控指标

| 指标 | 采集方式 | 告警阈值 | 说明 |
| --- | --- | --- | --- |
| 对账 count 差异 | 对账 Job | > 0 持续 2 个周期 | 可能存在数据丢失或补偿延迟 |
| 补偿队列深度 | 定时扫描补偿表 | > 100 持续 10 分钟 | 副路径写入异常 |
| 自愈操作次数 | 对账日志 | 单日 > 50 次 | 频繁自愈说明写入链路有问题 |
| 人工审批队列长度 | 审批表 | > 10 条 | 需人工介入的不一致积压 |
| `_id` 不一致计数 | 实时校验 | > 0 | 两侧 `_id` 应一致，不一致说明补偿异常（不应出现） |
| 关键字段不一致率 | 实时校验 + 对账 | > 0.1% | 数据质量下降 |

#### 3.17.8 各阶段一致性保障矩阵

| 阶段 | 写入保障 | 实时校验 | 定期对账 | 数据自愈 |
| --- | --- | --- | --- | --- |
| 迁移前 | 单写 Default | 无需 | 无需 | 无需 |
| 历史迁移（SYNC_JOB） | 双写 + 补偿 | ✅ 补偿后校验 | ✅ 每小时 count + 每天 checksum | ✅ 自动修复缺失/不一致 |
| 历史迁移（DBA_DUMP） | dump/restore 原子性 | ✅ dump/restore 后即时校验 | ✅ 每天 count + checksum | ✅ 自动修复（重新 dump/restore） |
| 历史迁移（NONE） | 双写 + 补偿（仅新数据） | ✅ 补偿后校验 | ✅ 每小时 count + 每天 checksum | ✅ 自动修复新数据缺失/不一致 |
| 双写期 | 双写 + 补偿 | ✅ 补偿后校验 | ✅ 每小时 count + 每天 checksum | ✅ 自动修复缺失/不一致 |
| 切流后 | 单写 Heavy | 无需（无双写） | ✅ 每天 count + 每周全量扫描 | ⚠️ 需人工审批 |
| 清理后 | 单写 Heavy | 无需 | ✅ 每周全量扫描 | ⚠️ 需人工审批 |
...
### 3.18 迁移期 file_reference 与 GC 约束

§3.14 将 `file_reference` 写入改为异步补偿。迁移期间通过 `migration.project-locks.freeze-gc` **暂停 GC** 作为主防线（§3.14.5、§3.18.2）；§3.14.5 的安全窗口+高优队列+二次确认仅作为正常运营期的防御纵深。
本节补充：**迁出项目在 `node_*` 两侧并存期间，`decrement` / 物理删除 / GC 同样存在一致性风险**。

#### 3.18.1 风险场景

```text
场景 A：双写 delete 不一致
T1: Heavy 上 deleteNode 成功 → file_reference decrement（Default）
T2: Default 副路径 delete 补偿尚未完成，Default 仍有 node 副本
T3: 某路径误扫 Default 副本再次 decrement → refCount 偏低 → 文件被错误 GC

场景 B：ROUTED 后僵尸副本
T1: 业务已只写 Heavy，Default 上 projectA 僵尸副本仍在
T2: Job 未带 projectId NOT IN 过滤，对 Default 僵尸副本做物理删除 + decrement
T3: Heavy 上 node 仍在，但 refCount 已减 → 文件被错误 GC
```

#### 3.18.2 迁移项目锁（`migration.project-locks`）

对迁出项目按阶段加锁，由 `NodeProjectSyncJob` / 配置中心维护：

```yaml
migration:
  project-locks:
    projectA:
      status: ROUTED          # 当前迁移阶段
      freeze-default-node-mutation: true   # 禁止 Default 侧 node 变更
      freeze-gc: true                      # 禁止该项目关联文件的物理 GC
      freeze-physical-delete: true         # 禁止 DeletedNodeCleanupJob 物理删
```

**各阶段锁策略**

> **核心决策：迁移全程暂停 `file_reference` GC。**
> 一旦代码切换到异步补偿模式（§3.14.2），`file_reference` 的 increment/decrement 变为异步——
> 补偿存在不可控的消费延迟，GC 无法保证在补偿完成前不会误判。
> §3.14.5 的三层防护（安全窗口 + 高优队列 + 二次确认）仅作为**正常运营期的防御纵深**，
> 迁移期间直接停止 GC 是最安全可靠的选择。单项目迁移周期通常在数天内完成，暂停 GC 产生的孤儿文件积压可忽略不计。

| 阶段 | `freeze-default-node-mutation` | `freeze-gc` | `freeze-physical-delete` | 说明 |
| --- | --- | --- | --- | --- |
| INITIAL_SYNC / CATCH_UP | false | true | false | 异步补偿已启用；GC 暂停防止补偿延迟导致误删文件 |
| DUAL_WRITE | true | true | false | 禁止直接改 Default 副本；GC 冻结防 decrement 偏差 |
| ROUTED ~ CLEANUP | true | true | true | 僵尸副本期：Default 不可碰 node；GC/物理删冻结 |
| CLEANED | false | false | false | Default 已清理，恢复正常 |

#### 3.18.3 decrement 单次原则

无论何阶段，`file_reference` 的 `decrement` **必须**满足：

1. 仅在一次**业务级 delete** 链路中触发一次（`DeletedNodeCleanupJob` 或 `deleteNode` API）。
2. delete 必须走路由层：迁出项目 delete 只打 Heavy 主路径；双写期副路径 Default delete 由补偿完成，**不重复 decrement**。
3. `DeletedNodeCleanupJob` 执行物理删除前检查 `migration.project-locks`：若 `freeze-physical-delete=true` 则跳过该行。

```kotlin
fun shouldPhysicalDelete(projectId: String): Boolean {
    val lock = migrationProjectLocks[projectId] ?: return true
    return !lock.freezePhysicalDelete
}

fun decrementRefCount(sha256: String, credentialsKey: String) {
    // 与 increment 对称：node 删除成功后写补偿任务，异步 decrement
    // 禁止在 Default 僵尸副本清理路径单独 decrement
}
```

#### 3.18.4 GC Job 改造要点

`file_reference` GC Job 在迁移期间**全局暂停**（通过 `migration.project-locks` 中 `freeze-gc=true` 控制），这是最可靠的防误删手段。

迁移完成（`CLEANED`）后 GC 恢复正常，此时依赖 §3.14.5 的三层防护（安全窗口 + 高优队列 + 二次确认）作为正常运营期的防御纵深。

| 防护 | 说明 |
| --- | --- |
| **迁移锁检查（主防线）** | 任一项目 `freeze-gc=true` 时，跳过其关联 `sha256` 的 GC 判定；迁移全程生效，从 INITIAL_SYNC 持续到 CLEANUP |
| 僵尸副本期 | ROUTED ~ CLEANUP 期间，GC 不得依据 Default 上迁出项目的 node 状态做判定（锁已兜底） |
| decrement 补偿优先 | `file_ref` decrement 补偿与 increment 共用高优先级队列 |

#### 3.18.5 `lastRefCountUpdate` 字段

GC 安全窗口依赖 `lastRefCountUpdate`（§3.14.5）。若字段不存在，实施前需：

```javascript
// 一次性回填（Default 实例）
db.file_reference_*.updateMany(
  { lastRefCountUpdate: { $exists: false } },
  { $set: { lastRefCountUpdate: "$createdDate" } }
)
```

后续所有 `increment` / `decrement`（含补偿消费）必须更新 `lastRefCountUpdate`。
...
**硬规则**：下列路径全部改造并验收通过后，方可启动模式二迁移编排（§10.5）。模式一 oplog **不受此限**。
...
> 完整版 **18 项**门禁见 **§25.5**（在下列 13 项基础上补充 Zombie 写保护、补偿入队兜底、writeConcern 校验、旁路对账、G-34）。

M7 首个大项目迁移前，以下项必须全部通过：

- [ ] 双写决策按 §3.5.1 实现（项目在 `project-routing` 中 + `dual-write=true`），`dual-write=false` 项目不误双写
- [ ] 进入 `DUAL_WRITE` 前 100% 新 Pod，无老 Pod 与双写并存（§3.10）
- [ ] 双写期迁出项目读 Default Secondary（§1.3、§3.6.2）
...
- [ ] `lastRefCountUpdate` 写路径与回填完成（§3.14.5、§3.18.5）
- [ ] 补偿队列同 `_id` 去重（`replaceOrAdd`）生效，同 `_id` 只保留最新补偿任务（§3.15.7）
- [ ] 补偿 update `lastModifiedDate` 使用 `$max` 保护，验证旧补偿不会降级时间戳（§3.15.7）
- [ ] `deleteRepository` / Pipeline 清理跨实例补偿改造完成（§20.3）
- [ ] 连接池总量未超 MongoDB `maxConnections` 阈值（§4.1）

### 3.20 完整配置流程（纯 Consul 驱动）

本节描述模式二从零到迁移完成的**完整配置操作序列**。所有运营态配置（`routingEnabled`、`dualWrite`、`projectRouting`）直接在 Consul 中修改即可。
...
│  │   rules.node.routing-enabled                                  │ │
│  │   rules.node.dual-write                                       │ │
...
# 3. 确认读取走 Heavy Secondary
# Heavy Secondary 上可观察到读流量
...
| 校验项 | 规则 | 不通过行为 |
|--------|------|-----------|
| configVersion 一致性 | `localVersion >= minConfigVersion` | 报警，等待 Pod 刷新 |
...
flowchart TD
    ...
    subgraph oplog [Oplog 实例 大磁盘+低CPU]
...
| Oplog 实例 | 大磁盘 + 低 CPU | 追加写为主，CPU 需求低，磁盘年增 ~270 GB |
...
```text
单 Pod 连接数 ≈ Default(主+从) + Σ Heavy实例(主+从) + Oplog(主+从)
...
#### 5.2.6 Service 层改造（file_reference 补偿，见 3.14 节）

| 类型 | 文件 | 改造内容 |
| --- | --- | --- |
| 修改 | `NodeBaseService.kt` | `doCreate` 中将 `fileReferenceService.increment` 同步调用改为写入补偿任务；移除 `@Transactional` 跨实例依赖 |
| 新建 | `FileRefCompensationTask.kt` | 补偿任务实体，字段见 3.14.3 |
| 新建 | `FileRefCompensationDao.kt` | 补偿任务的持久化，使用 Default 实例 MongoTemplate |
| 新建 | `FileRefCompensationJob.kt` | 定时消费补偿任务，调用 `fileReferenceService.increment`；重试上限告警 |
...
| --- | --- | --- | --- | --- |
| 初始（仅 Default + Oplog） | 2 | 100 | 10 | 2000 |
...
Oplog 实例同理：`OplogMongoConfiguration` 初始化时校验
...
| `POST /migration/dual-write` | 推送 Consul `dual-write=true` + `project-routing`（前置：§3.10 100% Pod、补偿清零、READY） |
| `POST /migration/route` | `dual-write=false`；补偿门禁后 ROUTED |
| `POST /migration/cleanup` | ROUTED → CLEANED |
| `POST /migration/rollback` | 回滚；按 projectId 清理 PENDING 补偿任务（G-29） |
| `GET /migration/status` | Job 阶段、CATCH_UP 延迟、`mongo_*_sync_failed` |
...
### 11.4 sha256→projectId 缓存层设计

当 `pageBySha256` 等查询可先确定目标 `projectId` 时，应优先精确路由，
避免散发所有实例。利用已有的 `file_reference_*` 集合构建轻量级缓存：

**缓存架构**：

```mermaid
flowchart TD
    A["pageBySha256(sha256)"] --> B{"本地缓存命中?"}
    B -- 是 --> C["按 projectId 集合\n精确路由到对应实例（可能多个）"]
    B -- 否 --> D["查询 file_reference_*\n（始终在 Default）"]
    D --> E{"查到 projectId?"}
    E -- 是 --> F["写入本地缓存\nTTL = 30 分钟"]
    F --> C
    E -- "否（sha256 无引用）" --> G["降级：散发查询所有实例"]
    C --> H["按 projectId 并发查询\n各实例 Secondary"]
    G --> I["散发查询合并去重\n见 §3.7"]

    style C fill:#e6f4ea,stroke:#34a853
    style G fill:#fffbe6,stroke:#d4a017
```
...
class Sha256ProjectIdCache(
    ...
    // Caffeine 本地缓存：30 分钟过期，最多 10000 条
    ...
    fun getProjectIds(sha256: String): Set<String>? {
        return cache.getIfPresent(sha256) ?: run {
            val refs = fileReferenceDao.findBySha256(sha256)
            if (refs.isNotEmpty()) {
                val projectIds = refs.map { it.projectId }.toSet()
                cache.put(sha256, projectIds)
                projectIds
            } else null
        }
    }

    /** 当 file_reference 变更时主动失效缓存 */
    fun invalidate(sha256: String) = cache.invalidate(sha256)
}
```

**缓存效果预估**：

| 场景 | 无缓存 | 有缓存命中（单 projectId） | 有缓存命中（多 projectId） |
| --- | --- | --- | --- |
| 散发实例数 | N（所有实例） | 1 | M（命中项目所在实例数） |
| P99 延迟 | max(N × 3s, 5s) | ~50ms | ~max(M × 3s) + merge |
| 适用条件 | 所有 pageBySha256 | 单项目引用同一 sha256 | 多项目共享同一 sha256 |

**缓存一致性保障**：

| 事件 | 动作 |
| --- | --- |
| `file_reference` 新增（increment） | **`invalidate(sha256)`**——同一 sha256 可能被新项目引用，缓存的 projectId 集合会过期 |
| `file_reference` 删除（decrement / refCount 归零） | `invalidate(sha256)` |
| 缓存 TTL 到期 | 下次查询回源 `file_reference_*` |
| `file_reference_*` 查询失败 | 降级到散发查询，不影响业务 |
| 缓存命中多个 projectId | 按 projectId 路由到**多个**实例并发查询后合并，不可只查单实例 |

> **注意**：缓存仅用于优化散发查询，不作为一致性依赖。未命中或失效时降级散发查询是安全的。
...
| 场景 | 操作 | 预期结果 |
| --- | --- | --- |
| Heavy 主库宕机（双写期） | kill Heavy primary | **迁出项目写入 fail-fast**；读仍可走 Default Secondary；恢复后业务重试 |
| Heavy 主库宕机（单写期） | kill Heavy primary | 目标项目写 fail-fast；`fallback-before-cleanup=true` 时可降级读 Default |
| Heavy 从库宕机 | kill Heavy secondary | 目标项目读 fail-fast |
| Default 从库宕机 | kill Default secondary | 全局非路由项目读不可用 |
...
| 散发查询随实例数增长线性退化 | 中 | 高实例数时 P99 不可接受 | sha256→projectId 索引优化；控制实例数 < 10 |
...
| 实例 | 规格参考 | 成本级别 | 说明 |
| --- | --- | --- | --- |
| Oplog 实例（副本集） | 4C8G + 500GB SSD × 3 节点 | 低 | 大磁盘低计算，存储为主 |
...
| 方案 | 一次性成本 | 月度增量 | 效果 |
| --- | --- | --- | --- |
| **分库（本方案）** | 开发 ~14 周 | 2~3 Heavy + 1 Oplog 实例 | 完全隔离 + 磁盘释放 |
...
class MongoRoutingRegistry(
    ...
    // 启动时按前缀长度降序构建查找表，优先匹配最长前缀，避免每次遍历
    private val prefixIndex: List<Pair<String, Pair<String, RoutingRule>>> by lazy {
        properties.rules.entries
            .filter { it.value.collectionPrefix.isNotBlank() }
            .map { it.value.collectionPrefix to (it.key to it.value) }
    ...
    fun routeWrite(collectionName: String, context: Any?): MongoTemplate? {
        val (ruleName, rule) = prefixIndex
            .firstOrNull { collectionName.startsWith(it.first) }?.second ?: return null
...
除 `file_reference_*`（§3.14 已覆盖）外，以下操作也可能涉及跨实例写入，
需逐一审计并确认分库后的事务安全性：
...
- **file_reference**：dst `increment` 走补偿模式（§3.14）；src 软删不立即 decrement，由 GC Job 处理
- **缓存**：移动完成后 `invalidate(sha256)`（G-37）
- **失败语义**：dst 已创建但 src 删除失败时，两端可能短暂共存，需运维对账或重试删除

| 操作 | 涉及集合 | 分库后位置 | 事务方式 | 风险 |
| --- | --- | --- | --- | --- |
| `createNode` | `node_*` + `file_reference_*` | Heavy + Default | 补偿（§3.14） | 已解决 |
| `moveNode` | `node_*`（src + dst） | 可能跨 Heavy 实例 | 先建后删，无跨实例事务 | 见上 |
| `copyNode` | `node_*` + `file_reference_*` | Heavy + Default | 补偿（§3.14） | 已解决 |
...
| Pipeline 清理 | `node_*` + `artifact_oplog_*` | Heavy + Oplog | 分步执行 + oplog 补偿 | 已解决（§20.3.2） |
...
`PipelineArtifactCleanupJob` 调用 `nodeService.deleteBeforeDate()` → `NodeDao` 路由层写 `node_*`；`artifact_oplog` 追加由 `OperateLogService` 写 Oplog 实例（模式一整体路由）。

| 步骤 | 实例 | 一致性要求 |
| --- | --- | --- |
| 删除 node | Heavy（迁出项目） | 主操作 |
| 追加 oplog | Oplog 实例 | 审计日志，**最终一致即可** |

**改造**：oplog 追加失败时写补偿任务（复用模式一补偿框架），不阻断 node 删除主路径。无需跨实例事务。
...
2. 跨实例写入是否已改为补偿模式（参照 `file_reference` §3.14）。
3. Job 写 `node_*` 是否显式传入 `projectId`。
...
| `readConcern`（读 Secondary） | **`majority`**（推荐）或 `local` | `majority` 保证不读到已回滚的数据；`local` 性能更好但对账/VERIFY 阶段必须用 `majority` |
| `readPreference`（业务读） | `secondaryPreferred` | 优先从库，降级到主库 |

**URI 示例**：

```
mongodb://default-primary:27017,default-secondary:27017/bkrepo?w=majority&readConcernLevel=majority&readPreference=secondaryPreferred
```
...
| --- | --- | --- |
| 70% | P2 告警 | 通知运维准备扩容 |
| 80% | P1 告警 + 加速清理 | 触发 TTL 清理加速、CLEANUP_READY 项目优先清理 |
| 85% | P0 告警 + 拒绝写入 | `migrationGate.close()`，拒绝迁入新项目 |
...
| G-23 | 补偿任务入队失败无兜底 | §3.12.5、§3.17.9 | 入队失败写本地文件兜底 + P0 告警；三级熔断增加入队失败路径 |
| G-24 | 多 Pod 并发消费补偿队列幂等性 | §3.15.5 | `findAndModify` 分布式锁（status PENDING → PROCESSING）+ 幂等消费 |
| G-25 | MongoDB writeConcern/readConcern 未声明 | §20a | 要求 writeConcern=majority；读 Secondary 使用 readConcern=majority；INIT 校验 |
...
| G-31 | Heavy 实例磁盘满渐进防护 | §3.12.4、§22 | 磁盘 70% 告警 / 85% 拒绝新写入；增量监控而非仅告警 |
| G-32 | Default oplog 容量预检查 | §3.9.2、§20a | INIT 阶段校验 oplog 保留窗口 ≥ 2× INITIAL_SYNC 预估耗时；文档给出最小值公式 |
| G-34 | 模式二分库前路由就绪 | §3.19.2、§10.5、§14 | P0 清单 + CI + 集成测试全通过；阻塞模式二迁移编排；模式一不受限 |
| G-39 | Heavy 磁盘满拒绝写入 | §3.12.4、§22 | ≥85% 时阻断该实例 node 新写入，不只拒新迁入 |
...
| G-37 | sha256 缓存路由变更失效 | §11.4 | binding 变更时 `invalidate(sha256)` |
| G-38 | `block_node` / `drive_node` 范围 | §1.2 | v1 不分库，留 Default |

**M7 上线前必查**（与 §3.19.3、§25.5 灰度门禁一致）：G-01、G-02、G-03、G-05、G-07、G-08、G-09、G-10、G-14、G-16、G-17、G-18、G-19、**G-23、G-24、G-25、G-26、G-34**（共 18 项，模式二）。
...
| E-23 | Heavy 实例磁盘满渐进防护 | 🟢 中等 | 低 | P2 | M7 |
...
**问题描述**（关联 G-02）：ROUTED 之后，若 Job 代码有 bug 或配置遗漏，未正确添加 `projectId NOT IN [已迁出]` 条件，会静默读写 Default 上的 zombie 副本。读 zombie 数据不更新 Heavy，写 zombie 数据造成静默数据分裂。
...
**解决方案**：补偿入队增加本地文件兜底 + P0 告警。

```kotlin
fun dualWrite(heavyWrite: () -> WriteResult, defaultWrite: () -> WriteResult) {
    heavyWrite() // 主路径，失败直接抛
    try {
        defaultWrite() // 同步尝试
    } catch (e: Exception) {
        try {
            compensationQueue.enqueue(task)
        } catch (enqueueEx: Exception) {
            // ⚠️ 最终防线失守
            log.error("CRITICAL: Compensation enqueue FAILED. Heavy has data, Default has none.", enqueueEx)
            alarm(P0, "补偿入队失败，Heavy/Default 将永久不一致！task=${task.primaryKey}")

            // 本地文件兜底（最坏情况的最后手段）
            val fallbackPath = Paths.get("/data/bkrepo/compensation_fallback")
            Files.createDirectories(fallbackPath)
            Files.writeString(
                fallbackPath.resolve("${task.primaryKey}_${System.currentTimeMillis()}.json"),
...
class CompensationFallbackRecovery(
    ...
    private val fallbackDir = Paths.get("/data/bkrepo/compensation_fallback")

    @Scheduled(fixedDelay = 60_000) // 每分钟扫描
    fun recoverFromFallback() {
        if (!Files.exists(fallbackDir)) return
        Files.list(fallbackDir).forEach { file ->
            try {
                val task = objectMapper.readValue(file.toFile(), CompensationTask::class.java)
                compensationDao.insertIfNotExists(task) // 幂等入队
                Files.delete(file) // 成功后删除本地文件
                log.info("Fallback recovery success: ${task.primaryKey}")
            } catch (e: Exception) {
                log.error("Fallback recovery failed: $file", e)
                // 文件保留，下次重试
            }
        }
    }
...
**验收标准**：
- 集成测试：Mock 补偿队列 MongoDB 不可用，验证本地文件写入 + P0 告警触发
- 恢复测试：Mock 恢复后补偿调度器扫描本地目录成功入队
- 三级熔断（§3.17.9）硬限制加入队失败降级路径
...
| 读 Secondary | `readConcern: local`（默认） | 可能读到已回滚的数据（Primary failover 后） |
...
| 业务读取（Secondary） | — | **majority**（推荐）或 local | majority 保证不读到回滚数据 |
...
### 24.24 E-23：Heavy 实例磁盘满渐进防护（P2）

**问题描述**（关联 G-31）：MongoDB 在磁盘使用率接近 100% 时**不会立即拒绝写入**，而是性能急剧下降。业务层表现为超时而非明确错误。双写期 Heavy 主路径超时 → 返回失败 → Default 副路径未触发 → 用户看到失败，但 Default 也未写入。
...
monitoring:
  disk:
    warning-threshold: 70%        # 告警
    reduce-retention-threshold: 80%  # 触发自动清理（TTL 过期数据加速清理）
    block-write-threshold: 85%    # 拒绝新写入（通过迁移锁 freeze-default-node-mutation）
```
...
class DiskUsageGuard(
    ...
    fun checkDiskUsage() {
        val diskUsage = getDiskUsage(template)
        when {
            diskUsage > 0.85 -> {
                alarm(P0, "Instance disk usage=$diskUsage, BLOCKING writes.")
                // 触发写保护：拒绝迁入新项目，加速现有项目的清理流程
                migrationGate.close()
                autoTriggerCleanup()
            }
            diskUsage > 0.80 -> {
                alarm(P1, "Instance disk usage=$diskUsage, accelerating cleanup.")
                // 触发 TTL 索引加速或触发 CLEANUP_READY 项目的清理
                accelerateCleanup()
            }
            diskUsage > 0.70 -> {
                alarm(P2, "Instance disk usage=$diskUsage.")
            }
        }
    }

    private fun getDiskUsage(template: MongoTemplate): Double {
        val status = template.mongoDbFactory.getMongoDatabase("admin")
            .runCommand(Document("serverStatus", 1))
        // 从 serverStatus 提取磁盘使用率
        return 0.0 // 实现略
    }
}
```
...
| E-23 | G-31 | Heavy 磁盘满渐进写保护——70/80/85% 三级防御 |
...
| 优先级 | 关联编号 | 修复章节 |
| --- | --- | --- |
| P0 | G-02、G-08、G-23、G-24；E-01、E-02、E-15 | §25.2 |
...
**修复**：改造 `MongoDualWriteCompensationService.enqueue`，入队失败时写本地文件 + P0 告警。
...
try {
    ...
    compensationFallbackWriter.write(task)   // 本地文件
    alarm(P0, "补偿入队失败")
}
```
...
| 组件 | 职责 |
| --- | --- |
| `CompensationFallbackWriter` | 写 `/data/bkrepo/compensation_fallback/{rule}_{routingKey}_{ts}.json` |
| `CompensationFallbackRecovery` | `@Scheduled(60s)` 扫描目录，幂等重试入队，成功后删文件 |
| `CompensationHealthController` | `GET /api/compensation/health/{ruleName}`（§24.3 E-02） |

**与三级熔断（§3.17.9）联动**：`replaceOrAdd` 替换已有 `_id` 任务时不计队列深度；入队失败路径独立 P0 告警，不静默吞掉。

**验收标准**：

- 集成测试：Mock 补偿队列 MongoDB 不可写，验证本地文件写入 + P0 告警
- 恢复测试：MongoDB 恢复后 `CompensationFallbackRecovery` 成功入队
- §25.5 第 15 项打勾
...
compensation:
  storage:
    uri: mongodb://compensation-primary:27017/bkrepo_compensation
    fallback-to-default: true    # 独立实例不可用时降级到 Default
  health-check:
    enabled: true
...
| 跨实例事务 | §20.3 | `deleteRepository`、Pipeline 清理分步 + 补偿 |
...
```yaml
spring.data.mongodb.multi-instance:
  config-version: 42           # 每次路由变更递增
  min-config-version: 42       # migrationGate 要求所有 Pod 达到此版本
```

```text
配置刷新后:
  localConfigVersion ← newVersion
  readiness probe: localVersion >= minRequiredVersion

migrationGate 进入 DUAL_WRITE 前:
  assertAllPodsSynced(requiredVersion)  // K8s 检查所有 Pod configVersion
```

配合 §3.10：进入 `DUAL_WRITE` 前 100% 新 Pod + 配置版本一致。

---

#### 25.3.4 G-25 / E-17：writeConcern 强制校验

**问题**：方案依赖「写入成功一定在 Secondary 可见」，但未文档化为强制实例配置（§20a、§24.18）。

**URI 标准**：

```text
mongodb://host/bkrepo?w=majority&readConcernLevel=majority&readPreference=secondaryPreferred
```
...
#### 25.3.6 G-03 / G-09：`file_reference` GC 三层防护落地

§3.14.5 / §3.18 设计已完整，M6 逐项落地：

| 层级 | 机制 | 配置项 |
| --- | --- | --- |
| 第〇层（迁移期主防线） | `migration.project-locks.freeze-gc=true` 时 GC 全局暂停 | `freeze-gc` |
| 第一层 | 文件创建 / refCount 变更后 T 分钟内跳过 GC | `file.reference.gc-safe-window-minutes`（默认 5） |
| 第二层 | `file_ref` 补偿独立高优先级线程池 | `compensation.file-ref.*` |
| 第三层 | 物理删除前二次查 `refCount` | GC Job 内实现 |

附加：`increment` / `decrement` 时更新 `lastRefCountUpdate`；历史数据一次性回填 Job（§3.18.5）。
...
| 3 | 双写期迁出项目读 Default Secondary | 集成测试写 Heavy 读 Default | 开发 |
...
| 9 | `lastRefCountUpdate` 写路径与回填完成（§3.14.5、§3.18.5） | GC 安全窗口集成测试 | 开发 |
...
| 12 | `deleteRepository` / Pipeline 清理跨实例补偿完成（§20.3） | 灰度验证 | 开发 |
...
| 15 | **补偿入队兜底**（§25.2.3、E-15） | Mock MongoDB 不可写，本地文件 + P0 告警 | 开发 |
| 16 | **writeConcern majority INIT 校验**（§25.3.4、E-17） | INIT 阶段校验通过 | 运维 |
| 17 | **旁路对账零差异**（§25.3.2、E-05） | 连续 3 轮抽样对账零差异 | 开发 |
| 18 | **G-34 路由就绪**（§3.19.2、§10.5） | `GET /routing/readiness` 全绿；P0 清单 100% | 开发 |

**M7 上线判定**：上表 18 项全部通过 + 模式一（oplog）已稳定运行 ≥ 2 周 + 小项目试点完整走通状态机（§3.9.1）。
...
| `common-mongo` | `MongoDualWriteCompensationService.kt` | 入队兜底 + `claimTask` + `replaceOrAdd` |
| `common-mongo` | `CompensationFallbackWriter.kt` | **新增** 本地文件兜底 |
| `common-mongo` | `CompensationFallbackRecovery.kt` | **新增** 定期扫描恢复 |
