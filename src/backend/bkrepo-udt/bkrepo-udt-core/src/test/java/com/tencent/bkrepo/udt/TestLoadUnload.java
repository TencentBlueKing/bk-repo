...
// gradle test的情况下测试不通过，但是单独的测试用例确能通过，所以暂时忽略掉
@Ignore
public class TestLoadUnload {
	...
	private static class JNIClassLoader extends ClassLoader {
		...
		protected Package getPackage(final String name) {
			if (name.equals("com.tencent.bkrepo.udt"))
		...
		protected synchronized Class<?> loadClass(final String name,
			...
			if (name.startsWith("com.tencent.bkrepo.udt."))
	...
	private int createSocketWithClassloader() throws ClassNotFoundException,
		...
		final File classPath = new File("build/classes/java/main/");

		JNIClassLoader loader = new JNIClassLoader(classPath);

		Class<Enum> typeClass = (Class<Enum>) loader
				.findClass("com.tencent.bkrepo.udt.TypeUDT");

		Class<?> socketClass = loader.findClass("com.tencent.bkrepo.udt.SocketUDT");
