...
class NodeBatchQueryHelperTest {
    ...
    fun `same shard scanned by Default with nin and Heavy with in for mixed routed projects`() {
        val collision = (0 until 10_000)
            .map { "p$it" }
            .groupBy { HashShardingUtils.shardingSequenceFor(it, SHARDING_COUNT) }
            .values
            .first { it.size >= 2 }
        val projectA = collision[0]
        val projectB = collision[1]
        val sameShard = shardFor(projectA)
        assertEquals(sameShard, shardFor(projectB))
        val collections = allNodeCollections()
        ...
        // 同分片时 sameShard 会同时出现在 Default(nin) 与 Heavy(in) 两组，靠 projectId 过滤互补
    }
}