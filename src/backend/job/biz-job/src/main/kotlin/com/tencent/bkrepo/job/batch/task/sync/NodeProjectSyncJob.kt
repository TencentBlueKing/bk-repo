...
class NodeProjectSyncJob(
    ...
    @Autowired(required = false) private val sidecarVerifier: DualWriteSidecarVerifier? = null,
    @Autowired(required = false) private val syncStateRepository: MigrationSyncStateRepository? = null,
) {
    ...
    fun run() {
        // 无论 routing-enabled 是否开启，只要有配置的实例就执行预同步
        val configuredProjects = registry.allConfiguredProjectsByInstance(NODE_RULE)
        if (configuredProjects.isEmpty()) return

        val states = loadAllSyncStates()
        configuredProjects.forEach { (instanceName, projects) ->
            projects.forEach { projectId ->
                val repoDoc = syncStateRepository?.findByProjectId(projectId)
                if (repoDoc?.phase == MigrationPhase.PENDING ||
                    repoDoc?.phase == MigrationPhase.INIT_FAILED
                ) {
                    return@forEach
                }
                if (states.none { it.projectId == projectId }) {
                    initSyncState(projectId, instanceName)
                }
            }
        }
    ...
    private fun advance(state: SyncStateDoc) {
        when (state.state) {
            ...
            SyncState.ROUTED -> advanceRoutingState(state)
    ...
    private fun advanceRoutingState(state: SyncStateDoc) {
        val routingEnabled = registry.isRoutingEnabled(NODE_RULE) &&
            registry.allConfiguredProjectsByInstance(NODE_RULE)
                .getOrDefault(state.targetInstance, emptySet())
                .contains(state.projectId)
        val dualWrite = registry.isDualWrite(NODE_RULE)
        when (state.state) {
            SyncState.READY -> when {
                !routingEnabled -> logger.debug("Project[${state.projectId}] READY, waiting for routing enable.")
                dualWrite -> {
                    // §25.3.3 / §25.3.2：进入 DUAL_WRITE 前门控校验
                    val gateResult = migrationGate?.canEnterDualWrite(
                        compensationQueueEmpty = !hasPendingDualWriteCompensations(state.projectId),
                        sidecarPassed = sidecarVerifier?.isRecentVerificationPassed(state.projectId, 3) ?: true,
                        currentDualWriteCount = countDualWriteProjects(),
                    )
                    if (gateResult != null && !gateResult.passed) {
                        logger.warn(
                            "Project[${state.projectId}] DUAL_WRITE gate blocked: $gateResult"
                        )
                        return
                    }
                    logger.info("Project[${state.projectId}] routing enabled with dual-write → DUAL_WRITE.")
                    updateState(state.projectId, SyncState.DUAL_WRITE)
                }
                else -> {
                    logger.info("Project[${state.projectId}] routing enabled without dual-write → ROUTED.")
                    updateState(state.projectId, SyncState.ROUTED)
                }
            }

            SyncState.DUAL_WRITE -> when {
                !routingEnabled -> {
                    logger.info("Project[${state.projectId}] routing disabled during DUAL_WRITE → READY.")
                    updateState(state.projectId, SyncState.READY)
                }
                dualWrite -> logger.debug("Project[${state.projectId}] DUAL_WRITE in progress.")
                // §25.3.3 / §25.3.2：DUAL_WRITE → ROUTED 门控：补偿清零 + 旁路对账连续3轮零差异
                !hasPendingDualWriteCompensations(state.projectId) &&
                    (sidecarVerifier?.isRecentVerificationPassed(state.projectId, 3) ?: true) -> {
                    val gateResult = migrationGate?.canSwitchToRouted(
                        compensationQueueEmpty = true,
                        sidecarPassed = sidecarVerifier?.isRecentVerificationPassed(state.projectId, 3) ?: true,
                    )
                    if (gateResult != null && !gateResult.passed) {
                        logger.warn(
                            "Project[${state.projectId}] ROUTED gate blocked: $gateResult"
                        )
                        return
                    }
                    if (hasSyncFailures(state.projectId)) {
                        logger.warn(
                            "Project[${state.projectId}] sync_failed not empty, delaying ROUTED."
                        )
                        return
                    }
                    logger.info("Project[${state.projectId}] DUAL_WRITE completed → ROUTED.")
                    updateState(state.projectId, SyncState.ROUTED)
                }
                else -> {
                    val reasons = mutableListOf<String>()
                    if (hasPendingDualWriteCompensations(state.projectId))
                        reasons += "compensation pending"
                    if (sidecarVerifier?.isRecentVerificationPassed(state.projectId, 3) == false)
                        reasons += "sidecar verification not passed"
                    if (reasons.isNotEmpty())
                        logger.info("Project[${state.projectId}] DUAL_WRITE waiting: $reasons")
                }
            }

            SyncState.ROUTED -> when {
                !routingEnabled -> {
                    logger.info("Project[${state.projectId}] routing disabled after ROUTED → READY.")
                    updateState(state.projectId, SyncState.READY)
                }
                dualWrite -> {
                    logger.info("Project[${state.projectId}] dual-write re-enabled → DUAL_WRITE.")
                    updateState(state.projectId, SyncState.DUAL_WRITE)
                }
                hasSyncFailures(state.projectId) -> logger.warn(
                    "Project[${state.projectId}] sync_failed not empty, delaying CLEANUP_READY."
                )
                System.currentTimeMillis() - state.updatedAt.toEpochSecond(java.time.ZoneOffset.UTC) * 1000 <
                    ROUTED_STABLE_WINDOW_MS -> {
                    logger.debug("Project[${state.projectId}] waiting for ROUTED stable window.")
                }
                else -> {
                    logger.info("Project[${state.projectId}] ROUTED stable enough → CLEANUP_READY.")
                    updateState(state.projectId, SyncState.CLEANUP_READY)
                }
            }
    ...
    private fun verifyWriteConcern(projectId: String, targetTemplate: MongoTemplate): Boolean {
        ...
        try {
            ...
            if (defaultMajor < 4 || heavyMajor < 4) {
                logger.error(
                    "Project[$projectId] MongoDB version too old: Default=$defaultVersion, Heavy=$heavyVersion (need ≥ 4.4)"
                )
                updateState(projectId, SyncState.INIT_FAILED,
        ...
        try {
            changeStream.cursor().use { cursor ->
                while (System.currentTimeMillis() < windowEnd) {
                    ...
                    if (event == null) {
                        if (System.currentTimeMillis() - lastEventTime >= LAG_QUIET_THRESHOLD_MS) {
                            // 达到静默阈值，认为已追上增量
                            break
                        }
                        continue
    ...
    private fun doVerify(state: SyncStateDoc) {
        ...
        logger.info("Project[$projectId] VERIFY passed all gates → READY.")
        NodeReconciliationHelper.persistLog(
            defaultMongoTemplate, projectId, "VERIFY", true, "all gates passed",
        )
        updateState(projectId, SyncState.READY)
    }
    ...
    private fun initSyncState(projectId: String, targetInstance: String) {
        syncStates[projectId] = SyncStateDoc(
            id = projectId,
            projectId = projectId,
        ...
        logger.info(
            "NodeProjectSyncJob: init sync state for project[$projectId] → instance[$targetInstance]"
        )
    }
    ...
    private fun countDualWriteProjects(): Int =
        syncStates.values.count { it.state == SyncState.DUAL_WRITE }
    ...
    private fun loadAllSyncStates(): List<SyncStateDoc> {
        syncStateRepository?.findByRuleName(NODE_RULE)?.forEach { doc ->
    ...
    private fun persistState(projectId: String) {
        val repo = syncStateRepository ?: return
    ...
    private fun hasPendingDualWriteCompensations(projectId: String): Boolean =
        defaultMongoTemplate.exists(
            Query(
                Criteria.where("ruleName").`is`(NODE_RULE)
                    .and("routingKey").`is`(projectId)
                    .and("status").ne("DONE")
            ),
            MongoDualWriteCompensationService.COLLECTION_NAME,
        )
    ...
    companion object {
        ...
        private const val SCHEDULE_INTERVAL_MS = 5 * 60 * 1000L
        /** Change Stream 服务端批次最大等待时间 */
        ...
        private const val ROUTED_STABLE_WINDOW_MS = 10 * 60 * 1000L
    }
}
