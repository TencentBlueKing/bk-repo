...
class MongoMigrationServiceMemoryDbTest {
    ...
    fun `binding then start advances phase PENDING to INITIAL_SYNC in real DB`() {
        ...
        assertEquals(MigrationPhase.PENDING, syncStateDao.findByRuleAndProject(ruleName, projectId)?.phase)

        service.start(MigrationProjectRequest(ruleName = ruleName, projectId = projectId))
        assertEquals(MigrationPhase.INITIAL_SYNC, syncStateDao.findByRuleAndProject(ruleName, projectId)?.phase)
    }
    ...
    fun `cleanup advances phase ROUTED to CLEANUP_READY in real DB`() {
        ...
        syncStateDao.upsert(
            TMigrationSyncState(
                id = MigrationSyncStateDao.resolveId("node", projectId),
                projectId = projectId,
                ruleName = "node",
        ...
        assertEquals(MigrationPhase.CLEANUP_READY, syncStateDao.findByRuleAndProject("node", projectId)?.phase)
    }
    ...
    fun `rollback after CLEANED returns 409 conflict fail-fast`() {
        ...
        assertEquals(MigrationPhase.CLEANED, syncStateDao.findByRuleAndProject("node", projectId)?.phase)
    }
    ...
    fun `rollback from ROUTED returns 409 conflict fail-fast`() {
        ...
        assertEquals(HttpStatus.CONFLICT, ex.status)
        assertEquals(MigrationPhase.ROUTED, syncStateDao.findByRuleAndProject("node", projectId)?.phase)
    }
    ...
    fun `cleanup of offload rule drops Default collection and sets CLEANED in real DB`() {
        ...
        assertEquals(MigrationPhase.CLEANED, syncStateDao.findByRuleAndProject(ruleName, projectId)?.phase)
