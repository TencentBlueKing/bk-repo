...
@Component
class ScanTriggeredEventPayloadBuilder : EventPayloadBuilder(EventType.SCAN_TRIGGERED) {
    override fun build(event: ArtifactEvent): CommonEventPayload {
        return ScanEventPayload(
            getUser(event.userId), EventType.SCAN_TRIGGERED, event.data["data"] as SubtaskEventPayload
        )
    }
}