...
class ArtifactFileRecordingListener(
    ...
    private val micAudioArtifactFile: ChunkedArtifactFile,
    private val syncMetadataArtifactFile: ChunkedArtifactFile,
    ...
    private lateinit var micAudioName: String
    private lateinit var syncMetadataName: String
    ...
    override fun init(name: String) {
        ...
        this.micAudioName = "MI_${fileName}.${MediaType.AAC.name.lowercase(Locale.getDefault())}"
        this.syncMetadataName = "AV_${fileName}.${MediaType.JSON.name.lowercase(Locale.getDefault())}"
    }
    ...
    override fun handler(packet: StreamPacket) {
        when (packet.getDataType()) {
            ...
            TYPE_MIC_AUDIO_DATA -> {
                micAudioArtifactFile.write(packet.getData())
            }

            TYPE_AV_MERGE_DATA -> {
                syncMetadataArtifactFile.write(packet.getData())
            }
    ...
    private fun storeFile(endTime: Long) {
        try {
            ...
            micAudioArtifactFile.finish()
            syncMetadataArtifactFile.finish()
            val extraFiles = buildExtraFilesMap()
            if (blockMode) {
            ...
            micAudioArtifactFile.delete()
            syncMetadataArtifactFile.delete()
        }
    }

    private fun buildExtraFilesMap(): MutableMap<String, ChunkedArtifactFile> {
        ...
        if (micAudioArtifactFile.getSize() != 0L) {
            extraFiles[micAudioName] = micAudioArtifactFile
        }
        if (syncMetadataArtifactFile.getSize() != 0L) {
            extraFiles[syncMetadataName] = syncMetadataArtifactFile
        }
        return extraFiles
    }
}
