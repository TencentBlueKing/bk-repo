...
class UrlDownloadBandwidthRateLimitRule(
    ...
    override fun addRateLimitRule(resourceLimit: ResourceLimit) {
        ...
        bandwidthLimitRules.addRateLimitRule(resourceLimit)
    }
    ...
    override fun filterResourceLimit(resourceLimit: ResourceLimit) {
        if (resourceLimit.limitDimension != LimitDimension.URL_DOWNLOAD_BANDWIDTH.name) {
            throw InvalidResourceException(resourceLimit.limitDimension)
        }
    }

    companion object {
        private val logger: Logger = LoggerFactory.getLogger(UrlDownloadBandwidthRateLimitRule::class.java)
    }
}
