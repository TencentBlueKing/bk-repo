 ...
 * URL上传带宽限流规则
 */
class UrlUploadBandwidthRateLimitRule(
    ...
    override fun filterResourceLimit(resourceLimit: ResourceLimit) {
        if (resourceLimit.limitDimension != LimitDimension.URL_UPLOAD_BANDWIDTH.name) {
            throw InvalidResourceException(resourceLimit.limitDimension)
        }
    }

    companion object {
        private val logger: Logger = LoggerFactory.getLogger(UrlUploadBandwidthRateLimitRule::class.java)
    }
}
