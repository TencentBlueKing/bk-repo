...
/**
 * URL前缀上传限速规则，pathLengthCheck=false 实现前缀匹配：
 * 配置 /proj/repo/bigfiles/ 可命中 /proj/repo/bigfiles/a.zip 等所有子路径。
 */
class UrlPrefixUploadBandwidthRateLimitRule : RateLimitRule {

    val uploadBandwidthLimitRules: PathResourceLimitRule = PathResourceLimitRule(pathLengthCheck = false)

    override fun isEmpty(): Boolean = uploadBandwidthLimitRules.isEmpty()

    override fun getRateLimitRule(resInfo: ResInfo): ResLimitInfo? {
        if (resInfo.resource.isBlank()) return null
        val ruleLimit = uploadBandwidthLimitRules.getPathResourceLimit(resInfo.resource) ?: return null
    ...
    override fun addRateLimitRule(resourceLimit: ResourceLimit) {
        ...
        uploadBandwidthLimitRules.addPathResourceLimit(resourceLimit, dimensionList)
    }
    ...
    override fun filterResourceLimit(resourceLimit: ResourceLimit) {
        if (resourceLimit.limitDimension != LimitDimension.URL_PREFIX_UPLOAD_BANDWIDTH.name) {
    ...
    companion object {
        private val logger: Logger = LoggerFactory.getLogger(UrlPrefixUploadBandwidthRateLimitRule::class.java)
        private val dimensionList = listOf(LimitDimension.URL_PREFIX_UPLOAD_BANDWIDTH.name)
    }
}
