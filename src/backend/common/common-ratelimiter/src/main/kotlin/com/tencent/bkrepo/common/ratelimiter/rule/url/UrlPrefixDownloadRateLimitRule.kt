...
class UrlPrefixDownloadRateLimitRule : RateLimitRule {

    val downloadUrlLimitRules: PathResourceLimitRule = PathResourceLimitRule()

    override fun isEmpty(): Boolean = downloadUrlLimitRules.isEmpty()

    override fun getRateLimitRule(resInfo: ResInfo): ResLimitInfo? {
        if (resInfo.resource.isBlank()) return null
        val ruleLimit = downloadUrlLimitRules.getPathResourceLimit(resInfo.resource) ?: return null
    ...
    override fun addRateLimitRule(resourceLimit: ResourceLimit) {
        ...
        downloadUrlLimitRules.addPathResourceLimit(resourceLimit, dimensionList)
    }
    ...
    override fun filterResourceLimit(resourceLimit: ResourceLimit) {
        if (resourceLimit.limitDimension != LimitDimension.URL_PREFIX_DOWNLOAD_RATE.name) {
    ...
    companion object {
        private val logger: Logger = LoggerFactory.getLogger(UrlPrefixDownloadRateLimitRule::class.java)
        private val dimensionList = listOf(LimitDimension.URL_PREFIX_DOWNLOAD_RATE.name)
    }
}
