...
class DefaultMongoRoutingRegistry(
    ...
    private val syncStateRepository: MigrationSyncStateRepository? = null,
    ...
    override fun historicalSyncStrategy(ruleName: String): String =
        properties.rules[ruleName]?.migration?.historicalSyncStrategy?.uppercase() ?: "JOB_ONLY"
    ...
    private val missingFieldCache = java.util.concurrent.ConcurrentHashMap.newKeySet<String>()
    ...
    override fun resolveByCollection(ruleName: String, collectionName: String): RoutedTemplate? {
        ...
        if (!rule.routingEnabled) return null
        val instanceName = rule.shardRouting[collectionName] ?: return null
        return toRoutedTemplate(ruleName, instanceName)
    }

    override fun resolveOplog(): RoutedTemplate {
        val (ruleName, rule) = properties.rules.entries.firstOrNull {
            it.value.collectionPrefix.startsWith(OPLOG_PREFIX)
        } ?: error("No oplog routing rule configured (collectionPrefix=$OPLOG_PREFIX)")
        val instanceName = rule.instances.keys.firstOrNull()
            ?: error("Oplog rule '$ruleName' has no instances")
        return toRoutedTemplate(ruleName, instanceName)
    }
    ...
    override fun getRoutedProjectIds(ruleName: String): Set<String> {
        ...
        if (!rule.routingEnabled) return emptySet()
        return rule.projectRouting.keys
    }
    ...
    override fun resolveReadRoute(
        ...
        if (!rule.routingEnabled) return ReadRoute(defaultTemplate)
        return when (rule.routingType) {
            MongoMultiInstanceProperties.RoutingType.NONE -> {
                if (rule.dualWrite) return ReadRoute(defaultTemplate)
                val instanceName = rule.instances.keys.firstOrNull() ?: return ReadRoute(defaultTemplate)
                val template = secondaryTemplates[ruleName]?.get(instanceName) ?: return ReadRoute(defaultTemplate)
                ReadRoute(
                    template = template,
    ...
    /**
     * 解析写操作路由，感知双写方向。
     * - NONE + dualWrite=true  → primary=Default，secondary=实例（Default 先写）
    ...
    override fun resolveWriteRoute(
        ...
        if (!rule.routingEnabled) return WriteRoute(defaultTemplate)
        return when (rule.routingType) {
            MongoMultiInstanceProperties.RoutingType.NONE -> {
                val instanceName = rule.instances.keys.firstOrNull()
                    ?: return WriteRoute(defaultTemplate)
    ...
    override fun isRoutingEnabled(ruleName: String): Boolean =
        properties.rules[ruleName]?.routingEnabled == true

    override fun isDualWrite(ruleName: String): Boolean =
        properties.rules[ruleName]?.let { it.routingEnabled && it.dualWrite } == true
    ...
    override fun projectsByInstance(ruleName: String): Map<String, Set<String>> {
        ...
        if (!rule.routingEnabled) return emptyMap()
    ...
    override fun secondaryTemplateByInstance(ruleName: String, instanceName: String): MongoTemplate? =
    ...
    /** 返回 instanceName → secondary MongoTemplate，供 Job 扫描读取使用 */
    override fun allSecondaryTemplates(ruleName: String): Map<String, MongoTemplate> =
    ...
    override fun isProjectRoutedOut(ruleName: String, projectId: String): Boolean {
        ...
        val phase = syncStateRepository?.findByProjectId(projectId)?.phase
        if (phase != null) {
            return phase.ordinal >= MigrationPhase.ROUTED.ordinal
        }
        // ponytail: 无状态记录时以 Consul dual-write=false 兜底
    ...
    override fun validateOnStartup() {
        val heavyCount = properties.rules.values
            .filter { it.routingEnabled && it.routingType == MongoMultiInstanceProperties.RoutingType.PROJECT }
    ...
    private fun toRoutedTemplate(ruleName: String, instanceName: String): RoutedTemplate {
        ...
        val secondary = secondaryTemplates[ruleName]?.get(instanceName)
        return RoutedTemplate(instanceId = instanceName, primary = primary, secondary = secondary)
    }
    ...
    private fun resolveTemplate(collectionName: String, context: Any?, primary: Boolean): MongoTemplate? {
        ...
        return when (rule.routingType) {
            MongoMultiInstanceProperties.RoutingType.NONE -> {
                ...
                if (rule.dualWrite) return null
                val instanceName = rule.instances.keys.firstOrNull() ?: return null
                templatesFor(ruleName, primary)[instanceName]
            }
            MongoMultiInstanceProperties.RoutingType.PROJECT,
            MongoMultiInstanceProperties.RoutingType.COLLECTION -> {
                val key = extractKey(context, rule.routingKeyField)
    ...
    private fun buildSecondaryTemplates(): Map<String, Map<String, MongoTemplate>> =
        properties.rules.entries
            .filter { it.value.routingEnabled }
            .associate { (ruleName, rule) ->
                ruleName to rule.instances.mapValues { (instanceName, cfg) ->
                    if (cfg.secondaryUri.isNotBlank()) {
                        MongoTemplate(createFactory(cfg.secondaryUri, cfg))
                    } else {
                        primaryTemplates[ruleName]?.get(instanceName)
                            ?: MongoTemplate(createFactory(cfg.uri, cfg))
                    }
