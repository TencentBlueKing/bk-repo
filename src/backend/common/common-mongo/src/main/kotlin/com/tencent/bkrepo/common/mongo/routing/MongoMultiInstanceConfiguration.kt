...
class MongoMultiInstanceConfiguration(
    ...
    private val syncStateRepository: ObjectProvider<MigrationSyncStateRepository>,
) {

    @Bean
    fun mongoRoutingRegistry(): MongoRoutingRegistry {
        val hasActiveRules = properties.rules.any { it.value.routingEnabled }
        val core = if (hasActiveRules) {
            DefaultMongoRoutingRegistry(
                properties,
                syncStateRepository.ifAvailable,
