 ...
 * 双写旁路对账器（§25.3.2 E-05）。
 *
 * 双写期间每 10 分钟对 DUAL_WRITE 项目随机抽样对比 Heavy vs Default 数据一致性。
 * 对账结果独立于补偿队列，作为切流门禁的第二道防线。
 *
...
class DualWriteSidecarVerifier(
    ...
    @Scheduled(fixedDelay = VERIFICATION_INTERVAL_MS)
    fun verify() {
        ...
        rule.forEach { (instanceName, projects) ->
            ...
            projects.forEach { projectId ->
                if (!isDualWriteProject(projectId)) return@forEach
                try {
    ...
    companion object {
        ...
        private const val VERIFICATION_INTERVAL_MS = 600_000L
