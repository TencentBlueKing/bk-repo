...
/**
 * 迁移门控（§25.3.3 E-06 / §3.10）。
 *
 * 在所有迁移状态跃迁前执行前置条件校验，任一不满足则拒绝跃迁。
 *
 * 门控校验链：
 * - DUAL_WRITE 前：100% Pod configVersion ≥ minConfigVersion + 补偿队列清零 + 旁路对账连续3轮零差异
...
class MigrationGate(
    ...
    fun isZombieReplicaOverdue(ruleName: String, projectId: String): Boolean {
        ...
        val state = syncStateRepository?.findByProjectId(projectId) ?: return false
    ...
    private fun checkConfigVersion(): GateCheck {
        val minVersion = registry.getMinConfigVersion()
        val cluster = podRegistry?.verifyClusterUpToDate(minVersion)
        if (cluster != null && !cluster.passed) {
            return GateCheck(
                name = "configVersion",
                passed = false,
                reason = cluster.reason ?: "cluster pod config-version check failed",
            )
        }
        val upToDate = registry.isConfigUpToDate()
        return GateCheck(
            name = "configVersion",
            passed = upToDate,
            reason = if (!upToDate) {
                "configVersion=${registry.getConfigVersion()} < minConfigVersion=$minVersion"
            } else null,
        )
    }
