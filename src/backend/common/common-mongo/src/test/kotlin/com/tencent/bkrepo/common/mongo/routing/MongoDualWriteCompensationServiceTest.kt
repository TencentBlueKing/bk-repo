...
class MongoDualWriteCompensationServiceTest {
    ...
    @Test
    fun `claimTasks skips pending task whose nextRetryAt is in the future`() {
        val futureTask = buildCompensationDoc(
            status = "PENDING",
            operationType = "SAVE",
            collectionName = "node_future",
        ).apply {
            put("nextRetryAt", LocalDateTime.now().plusSeconds(30))
        }
        mongoTemplate.insert(futureTask, compensationCollection)

        val claimed = service.claimTasks(1)

        assertTrue(claimed.isEmpty())
        val updated = mongoTemplate.findById(
            futureTask.getObjectId("_id"),
            Document::class.java,
            compensationCollection,
        )
    ...
    @Test
    fun `claimTasks still claims pending task whose nextRetryAt is null`() {
        val task = buildCompensationDoc(
            status = "PENDING",
            operationType = "SAVE",
            collectionName = "node_null_retry",
        ).apply {
            put("nextRetryAt", null)
        }
        ...
        assertEquals(task.getObjectId("_id"), claimed.first().getObjectId("_id"))
        val updated = mongoTemplate.findById(
    ...
    fun `resetStaleProcessing resets stale PROCESSING tasks to PENDING`() {
        // 插入一个超时的 PROCESSING 任务 (claimedAt = 600s 前，超过 300s timeout)
        val staleDoc = buildCompensationDoc(
            status = "PROCESSING",
            claimedAt = LocalDateTime.now().minusSeconds(600),
        )
        ...
        assertNull(resetDoc.getString("claimedBy"))
        assertNull(resetDoc.get("claimedAt"))
    ...
    @Test
    fun `replay schedules first retry at about now plus ten seconds`() {
        val entityId = ObjectId()
        mongoTemplate.insert(
            Document().apply { put("_id", entityId); put("name", "existing") },
            replayTargetCollection,
        ...
        val task = buildCompensationDoc(
            ...
            collectionName = replayTargetCollection,
            entityClass = TestEntity::class.java.name,
            entity = Document().apply {
                put("_id", entityId)
                put("name", "retry-delay")
            },
            retryCount = 0,
            targetUseDefault = true,
        )
    ...
    // ── 17. replaceOrAdd CAS（PROCESSING 不合并）────────────────

    @Test
    fun `replaceOrAdd appends new task when existing task is PROCESSING`() {
        val docId = ObjectId()
        val query = Query(Criteria.where("_id").`is`(docId))
        val route = writeRoute()
        service.enqueueUpdateFirst(
            route, "node_0", query,
            org.springframework.data.mongodb.core.query.Update().inc("count", 1),
        )
        mongoTemplate.updateFirst(
            Query(Criteria.where("primaryKey").`is`(docId.toHexString())),
            org.springframework.data.mongodb.core.query.Update().set("status", "PROCESSING"),
            compensationCollection,
        )
        service.enqueueUpdateFirst(
            route, "node_0", query,
