 ...
 * @date: 2019/09/09
 */
abstract class AbstractFileStorage<Credentials : StorageCredentials, Client>(
    ...
    override fun store(hash: String, inputStream: InputStream, storageCredentials: StorageCredentials?) {
        val path = locateStrategy.locate(hash)
        val credentials = getOrDefaultCredentials(storageCredentials)
    ...
    override fun delete(hash: String, storageCredentials: StorageCredentials?) {
        val path = locateStrategy.locate(hash)
        val credentials = getOrDefaultCredentials(storageCredentials)
    ...
    override fun load(hash: String, storageCredentials: StorageCredentials?): File? {
        val path = locateStrategy.locate(hash)
        val credentials = getOrDefaultCredentials(storageCredentials)
    ...
    override fun exist(hash: String, storageCredentials: StorageCredentials?): Boolean {
        val path = locateStrategy.locate(hash)
        val credentials = getOrDefaultCredentials(storageCredentials)
        return try {
    ...
    override fun append(path: String, filename: String, inputStream: InputStream, storageCredentials: StorageCredentials?) {
        val credentials = getOrDefaultCredentials(storageCredentials)
        try {
    ...
    override fun storeBlock(path: String, sequence: Int, sha256: String, inputStream: InputStream, storageCredentials: StorageCredentials?) {
        val credentials = getOrDefaultCredentials(storageCredentials)
        try {
    ...
    override fun combineBlock(path: String, storageCredentials: StorageCredentials?): File {
        val credentials = getOrDefaultCredentials(storageCredentials)
        try {
            val client = getClient(credentials)
    ...
    override fun listBlockInfo(path: String, storageCredentials: StorageCredentials?): List<Pair<Long, String>> {
        val credentials = getOrDefaultCredentials(storageCredentials)
        try {
            val client = getClient(credentials)
    ...
    override fun makeBlockPath(path: String, storageCredentials: StorageCredentials?) {
        val credentials = getOrDefaultCredentials(storageCredentials)
        try {
            val client = getClient(credentials)
    ...
    override fun checkBlockPath(path: String, storageCredentials: StorageCredentials?): Boolean {
        val credentials = getOrDefaultCredentials(storageCredentials)
        try {
            val client = getClient(credentials)
    ...
    override fun deleteBlockPath(path: String, storageCredentials: StorageCredentials?) {
        val credentials = getOrDefaultCredentials(storageCredentials)
        try {
            val client = getClient(credentials)
    ...
    private fun getClient(storageCredentials: StorageCredentials): Client {
        val credentials = storageCredentials as Credentials
        return clientCache?.get(credentials) ?: createClient(credentials)
    }

    private fun getOrDefaultCredentials(storageCredentials: StorageCredentials?): StorageCredentials {
        return storageCredentials ?: properties.credentials
    }
