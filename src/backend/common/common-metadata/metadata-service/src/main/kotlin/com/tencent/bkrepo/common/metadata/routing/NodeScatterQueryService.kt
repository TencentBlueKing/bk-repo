...
class NodeScatterQueryService(
    ...
    private val projectIdCache: Sha256ProjectIdCache? = null,
) {
    ...
    fun pageBySha256(
        ...
        val cachedProjects = projectIdCache?.get(sha256)
        val all = if (!cachedProjects.isNullOrEmpty()) {
            findBySha256ForProjects(sha256, cachedProjects, collectionNames)
        } else {
            val query = sha256Query(sha256)
                .limit((offset + pageRequest.pageSize).toInt().coerceAtMost(MAX_SCATTER_FETCH))
            scatterFind(query, TNode::class.java, collectionNames).also { nodes ->
                projectIdCache?.put(sha256, nodes.map { it.projectId }.toSet())
            }
        }
    ...
    private fun findBySha256ForProjects(
        sha256: String,
        projectIds: Set<String>,
        collectionNames: List<String>,
    ): List<TNode> {
        val started = System.nanoTime()
        val routed = registry.routedProjectIds(NODE_RULE)
        val instanceByProject = registry.projectsByInstance(NODE_RULE)
            .flatMap { (instance, projects) -> projects.map { it to instance } }
            .toMap()
        val futures = projectIds.map { projectId ->
            CompletableFuture.supplyAsync(
                {
                    val collection = collectionForProject(projectId, collectionNames)
                    val template = if (projectId in routed) {
                        val instance = instanceByProject[projectId]
                        instance?.let { heavyReadTemplate(it) }
                    } else {
                        defaultReadTemplate()
                    } ?: defaultReadTemplate()
                    val query = sha256Query(sha256)
                        .addCriteria(Criteria.where(PROJECT_FIELD).`is`(projectId))
                    findOnTemplate(template, query, TNode::class.java, collection)
                },
                executor,
            )
        }
    ...
    private fun heavyReadTemplate(instanceName: String): MongoTemplate? =
        scatterTemplates?.heavyTemplate(instanceName)
            ?: registry.secondaryTemplateByInstance(NODE_RULE, instanceName)
    ...
    private fun collectionForProject(projectId: String, collectionNames: List<String>): String {
        val shard = "$NODE_PREFIX${HashShardingUtils.shardingSequenceFor(projectId, SHARDING_COUNT)}"
        return shard.takeIf { it in collectionNames } ?: collectionNames.first()
    }
    ...
    private fun buildScatterGroups(collectionNames: List<String>): List<ScatterGroup> {
        ...
        val defaultPatch: (Query) -> Query = if (routedProjects.isNotEmpty()) {
            if (routedProjects.size > WHITELIST_THRESHOLD) {
                val remaining = registry.allKnownProjectIds(NODE_RULE) - routedProjects
                if (remaining.isNotEmpty()) {
                    { q -> Query.of(q).addCriteria(Criteria.where(PROJECT_FIELD).`in`(remaining)) }
                } else {
                    { it }
                }
            } else {
