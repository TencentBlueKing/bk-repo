...
class FederationMetricsCollector(
    ...
    fun recordFullSync(
        ...
        filesTransferred: Long = 0L
    ) {
        val tags = mapOf(TAG_PROJECT_ID to projectId, TAG_REPO_NAME to repoName)

        // 记录全量同步总数
        getOrCreateCounter(METRIC_FULLSYNC_TOTAL, DESC_FULLSYNC_TOTAL, tags).increment()

        // 记录成功或失败
        val resultTags = tags + (TAG_SYNC_RESULT to if (success) RESULT_SUCCESS else RESULT_FAILED)
        if (success) {
            getOrCreateCounter(METRIC_FULLSYNC_SUCCESS, DESC_FULLSYNC_SUCCESS, resultTags).increment()
        } else {
            getOrCreateCounter(METRIC_FULLSYNC_FAILED, DESC_FULLSYNC_FAILED, resultTags).increment()
        }

        // 记录耗时
        getOrCreateTimer(METRIC_FULLSYNC_DURATION, DESC_FULLSYNC_DURATION, tags)
            .record(durationMillis, TimeUnit.MILLISECONDS)

        // 记录传输字节数
        if (bytesTransferred > 0) {
            getOrCreateDistributionSummary(METRIC_FULLSYNC_BYTES, DESC_FULLSYNC_BYTES, UNIT_BYTES, tags)
                .record(bytesTransferred.toDouble())
        }

        // 记录传输文件数
        if (filesTransferred > 0) {
            getOrCreateDistributionSummary(METRIC_FULLSYNC_FILES, DESC_FULLSYNC_FILES, UNIT_FILES, tags)
                .record(filesTransferred.toDouble())
        }
    }
