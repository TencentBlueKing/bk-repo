...
class DockerV2Errors {
    companion object {
        ...
        fun blobUnknown(digest: String): ResponseEntity<Any> {
            return ResponseEntity.status(404).header("Docker-Distribution-Api-Version", "registry/2.0")
                .contentType(MediaType.APPLICATION_JSON).header("Content-Length", "157").body(
                String.format(
                    "{\"errors\":[{\"code\":\"%s\",\"message\":\"%s\",\"detail\":{%s}}]}",
                    "BLOB_UNKNOWN",
        ...
        fun unauthorized(
            ...
            return ResponseEntity.status(401).header("Docker-Distribution-Api-Version", "registry/2.0").header(
                "WWW-Authenticate",
                String.format("Bearer realm=\"%s\",service=\"%s\"", tokenUrl, registryService) + scopeStr
            ).contentType(MediaType.APPLICATION_JSON).body(
                String.format(
                    "{\"errors\":[{\"code\":\"%s\",\"message\":\"%s\",\"detail\":null}]}",
                    "UNAUTHORIZED",
        ...
        fun unauthorizedManifest(manifest: String, err: String?): ResponseEntity<Any> {
            return ResponseEntity.status(403).header("Docker-Distribution-Api-Version", "registry/2.0")
                ...
                String.format(
                    "{\"errors\":[{\"code\":\"%s\",\"message\":\"%s\",\"detail\":{%s}}]}",
                    "UNAUTHORIZED",
                    "The client does not have permission for manifest" + if (err != null) ": $err" else "",
                    "\"manifest\":\"$manifest\""
                )
            )
        }
