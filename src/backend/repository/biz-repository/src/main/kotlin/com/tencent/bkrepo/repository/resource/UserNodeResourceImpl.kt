...
class UserNodeResourceImpl @Autowired constructor(
    ...
    override fun detail(userId: String, artifactCoordinate: ArtifactCoordinate): Response<NodeDetail?> {
        artifactCoordinate.run {
            ...
            val nodeDetail = nodeService.detail(projectId, repoName, artifactPath.fullPath) ?: throw ErrorCodeException(
                CommonMessageCode.ELEMENT_NOT_FOUND, artifactPath.fullPath)
    ...
    override fun computeSize(userId: String, artifactCoordinate: ArtifactCoordinate): Response<NodeSizeInfo> {
        artifactCoordinate.run {
            permissionService.checkPermission(CheckPermissionRequest(userId, ResourceType.REPO, PermissionAction.READ, projectId, repoName))
            val nodeSizeInfo = nodeService.computeSize(projectId, repoName, artifactPath.fullPath)
