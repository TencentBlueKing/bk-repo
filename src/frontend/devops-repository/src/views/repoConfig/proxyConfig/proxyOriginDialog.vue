        ...
        methods: {
            ...
            async testConnection () {
                ...
                if (this.editProxyData.proxyType === 'privateProxy') {
                    const auth = 'Basic ' + Base64.encode(this.editProxyData.username + ':' + this.editProxyData.password)
                    const response = await fetch(this.editProxyData.url + '/index.yaml', {
                        headers: {
                            Authorization: auth
                        }
                    }).catch(() => {
                    ...
                    const response = await fetch(this.editProxyData.url + '/index.yaml').catch(() => {
                        this.connected = false
                    }).finally(
                        this.loading = false
                    )
